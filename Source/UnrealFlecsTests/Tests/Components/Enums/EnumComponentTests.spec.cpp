...
void FEnumComponentTestsSpec::Define()
{
	...
	Describe("Sparse Enum Components", [this]()
	{
		...
		It("Should Remove an Enum Component using Static API with Sparse UENUM", [this]()
		{
			...
			TestFalse("Entity has Enum Component", TestEntity.Has(StaticEnum<ETestEnum_SparseUENUM>()));
			TestFalse("Entity has Enum Component", TestEntity.HasPair<ETestEnum_SparseUENUM>(flecs::Wildcard));
		});
