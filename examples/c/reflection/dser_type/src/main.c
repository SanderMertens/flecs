...
int main(int argc, char *argv[]) {
    ...
    ecs_vec_init(NULL, &value.v3, sizeof(ecs_f64_t), 100);
    printf("v1: %i of %i\n", ecs_vec_count(&value.v1), ecs_vec_size(&value.v1));
    printf("v2: %i of %i\n", ecs_vec_count(&value.v2), ecs_vec_size(&value.v2));
    printf("v3: %i of %i\n", ecs_vec_count(&value.v3), ecs_vec_size(&value.v3));
    char const * json_input = "{\"v1\": [], \"v3\":[], \"v2\": [5.123, 6.123, 1, 3, 6, 7, 5, 8, 3, 7, 4]}";
    ...
    if(ptr)
    {
        ...
        printf("json_input: %s\n", json_input);
        //json_input: {"v1": [], "v3":[], "v2": [5.123, 6.123, 1, 3, 6, 7, 5, 8, 3, 7, 4]}
        printf("json_output: %s\n", json_output);
        //json_output: {"v1":[], "v2":[5.123, 6.123, 1, 3, 6, 7, 5, 8, 3, 7, 4], "v3":[]}
    }
