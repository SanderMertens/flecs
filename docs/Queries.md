# Flecs Queries
Queries are the mechanism that allow applications to efficiently get the entities that match with a certain set of conditions. Queries can range from simple lists of components to complex expressions that efficiently traverse an entity graph. This manual explains the ins & outs of how to use them.
