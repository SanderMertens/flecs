...
A pair is an id that encodes two elements. Pairs, like components, can be added to entities and are the foundation for [Relationships](Relations.md).
...
auto q = world.query_builder<const Depth, Position>()
  .order_by([](flecs::entity e1, const Depth *d1, flecs::entity e2, const Depth *d2) {
    return (d1->value > d2->value) - (d1->value < d2->value);
  })
  .build();
```
...
auto q = world.query_builder<Position>()
  .term(depth_id).in()
  .order_by(depth_id, [](flecs::entity e1, const void *d1, flecs::entity e2, const void *d2) {
    // Generic sort code ...
  })
  .build();
```
...
It is possible to prevent inheriting from a component from by adding the [Final](Relations.md#final-property) property. Queries for components with the `Final` property will not attempt to resolve component inheritance.
...
When a [transitive relationship](Relations.md#transitive-relationships) is used by a query, a query will automatically look up or test against pairs that satisfy the transitive property. Transitivity is usually defined as:
...
When a query matches a [relfexive](Relations.md#reflexive-property) relationship, a query term will evaluate to true if the source and target are equal. Reflexivity can be defined as:
