...
Hierarchies can be traversed breadth-first by using the `cascade` feature of [queries](https://www.flecs.dev/flecs/Queries.html#relationship-traversal). For an example, see:
...
For more details, see the [names section](https://www.flecs.dev/flecs/EntitiesComponents.html#names) in the Entities and Components manual.
...
- [Prefab instantiation](https://www.flecs.dev/flecs/PrefabsManual.html#prefab-hierarchies) works with both storages.
- [JSON serialization](https://www.flecs.dev/flecs/FlecsRemoteApi.html#json-serialization) works for both storages.
...
- The `Or` and `Not` [query operators](https://www.flecs.dev/flecs/Queries.html#operator-overview) do not yet work with `Parent` hierarchies.
