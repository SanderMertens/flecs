...
### Ranges
Entity ranges require the `FLECS_ENTITY_RANGES` addon. The addon is enabled in
the default build and must be selected explicitly in a custom build.
...
Enabling or disabling a prefab affects the prefab entity itself. It does not
enable or disable the ids stored on the prefab, its base prefabs, or its
children.
