...
static void Await_store_callback(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_script_future_t *future)
...
static void Await_store_string_callback(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_script_future_t *future)
...
static void Await_record_callback(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_script_future_t *future)
...
static void Await_immediate_callback(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_script_future_t *future)
...
static void Await_method_placeholder(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_value_t *result)
...
static void Await_method_callback(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_script_future_t *future)
...
static void Await_cancel_callback(
    const ecs_function_ctx_t *ctx,
    ecs_script_future_t *future)
{
    (void)ctx;
...
static void Await_future_ctx_free(
    void *ctx)
{
...
static void Await_store_with_ctx_callback(
    const ecs_function_ctx_t *ctx,
    int32_t argc,
    const ecs_value_t *argv,
    ecs_script_future_t *future)
...
void Await_entity_enter_error_restores_state(void) {
    ...
    visitor.base.next = (ecs_script_node_t*)node;
...
void Await_task_component_added_on_task_new(void) {
    ...
    test_assert(t != NULL);
    test_int(ecs_vec_count(&t->tasks), 1);
    test_assert(*ecs_vec_first_t(&t->tasks, ecs_script_task_t*) == task);
...
void Await_task_component_not_added_without_entity(void) {
    ...
    ecs_script_task_t *task = ecs_script_task_new(script, NULL);
    test_assert(task != NULL);
    test_int(ecs_count(world, EcsScriptTask), count_before);
...
void Await_task_component_two_tasks(void) {
    ...
    const EcsScriptTask *t = ecs_get(world, e, EcsScriptTask);
    test_assert(t != NULL);
    test_int(ecs_vec_count(&t->tasks), 2);

    ecs_script_task_free(task_a);
    test_assert(ecs_has(world, e, EcsScriptTask));
    t = ecs_get(world, e, EcsScriptTask);
    test_int(ecs_vec_count(&t->tasks), 1);
    test_assert(*ecs_vec_first_t(&t->tasks, ecs_script_task_t*) == task_b);
...
void Await_task_component_serialize(void) {
    ...
    test_str(json,
        ...
        "\"line\":2, \"column\":1, "
        "\"awaiting\":\"fetch\", \"iteration\":0}]");
    ecs_os_free(json);

    int32_t v = 10;
    test_int(0, ecs_script_future_resolve(await_futures[0],
        &(ecs_value_t){ .type = ecs_id(ecs_i32_t), .ptr = &v }));
    ecs_script_future_release(await_futures[0]);
    test_int(ecs_script_task_resume(task, NULL), EcsScriptTaskDone);

    json = ecs_ptr_to_json(world, ecs_id(EcsScriptTask), t);
    test_assert(json != NULL);
    test_str(json,
        "[{\"script\":\"task_script\", \"status\":\"Done\", "
        "\"line\":0, \"column\":0, "
        "\"awaiting\":\"#0\", \"iteration\":0}]");
    ecs_os_free(json);
...
void Await_task_component_deferred_three_tasks(void) {
    ...
    ecs_script_task_t *task_b = ecs_script_task_new(script,
        &(ecs_script_task_desc_t){ .entity = e });
    ecs_script_task_t *task_c = ecs_script_task_new(script,
        &(ecs_script_task_desc_t){ .entity = e });
    ...
    const EcsScriptTask *t = ecs_get(world, e, EcsScriptTask);
    test_assert(t != NULL);
    test_int(ecs_vec_count(&t->tasks), 3);

    ecs_script_task_t **tasks = ecs_vec_first(&t->tasks);
    test_assert(tasks[0] == task_a);
    test_assert(tasks[1] == task_b);
    test_assert(tasks[2] == task_c);
    ...
    t = ecs_get(world, e, EcsScriptTask);
    test_int(ecs_vec_count(&t->tasks), 1);
    test_assert(*ecs_vec_first_t(&t->tasks, ecs_script_task_t*) == task_c);

    ecs_script_task_free(task_c);
    test_assert(!ecs_has(world, e, EcsScriptTask));
...
void Await_many_live_strings_resume_and_cancel(void) {
    ...
    ecs_strbuf_t buf = ECS_STRBUF_INIT;
    for (int i = 0; i < 640; i ++) {
        ecs_strbuf_append(&buf, "const v%d = \"value_%d\"\n", i, i);
    }
    ecs_strbuf_appendstr(&buf, "await step()\n");
    for (int i = 0; i < 640; i ++) {
        ecs_strbuf_append(&buf, "\"{$v%d}\" {}\n", i);
    }
    char *code = ecs_strbuf_get(&buf);
    ecs_script_t *script = ecs_script_parse(world, NULL, code, &ir_desc, NULL);
    ecs_os_free(code);
    ...
    for (int pass = 0; pass < 2; pass ++) {
        ...
        test_assert(task != NULL);
        test_int(ecs_script_task_resume(task, NULL), EcsScriptTaskPending);
        test_int(await_future_count, pass + 1);
        if (!pass) {
            ecs_script_task_cancel(task);
            test_int(await_cancel_count, 1);
            test_assert(ecs_script_future_is_cancelled(await_futures[pass]));
            test_uint(ecs_lookup(world, "value_0"), 0);
        } else {
            ecs_value_t value = ecs_value(ecs_i32_t, {42});
            test_int(ecs_script_future_resolve(await_futures[pass], &value), 0);
            test_int(ecs_script_task_resume(task, NULL), EcsScriptTaskDone);
            for (int i = 0; i < 640; i ++) {
                char name[32];
                snprintf(name, sizeof(name), "value_%d", i);
                test_assert(ecs_lookup(world, name) != 0);
            }
        }
        ecs_script_future_release(await_futures[pass]);
        ecs_script_task_free(task);
    }
    ecs_script_free(script);
    ecs_fini(world);
