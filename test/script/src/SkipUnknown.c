#include <script.h>

static bool ir_enabled = false;
static ecs_script_eval_desc_t ir_desc = {0};
static ecs_script_eval_desc_t skip_unknown_desc = {0};

void SkipUnknown_setup(void) {
    const char *ir_param = test_param("ir");
    ir_enabled = ir_param && !strcmp(ir_param, "enabled");
    ir_desc = (ecs_script_eval_desc_t){ .ir = ir_enabled };
    skip_unknown_desc = (ecs_script_eval_desc_t){ .ir = ir_enabled, .skip_unknown = true };
}

static int skip_unknown_warn_count = 0;

static void skip_unknown_warn_callback(
    int32_t level,
    const char *file,
    int32_t line,
    const char *msg)
{
    (void)file;
    (void)line;
    (void)msg;
    if (level == -2) {
        skip_unknown_warn_count ++;
    }
}

static void skip_unknown_warn_capture(void) {
    ecs_os_set_api_defaults();
    ecs_os_api_t os_api = ecs_os_api;
    os_api.log_ = skip_unknown_warn_callback;
    ecs_os_set_api(&os_api);
    ecs_log_set_level(-2);
    skip_unknown_warn_count = 0;
}

static void skip_unknown_warn_release(void) {
    ecs_os_set_api_defaults();
    ecs_log_set_level(-1);
}

void SkipUnknown_unknown_tag(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownTag"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup(world, "NsUnknownTag") == 0);

    const ecs_type_t *type = ecs_get_type(world, e);
    test_assert(type != NULL);
    test_int(type->count, 1); /* (Identifier, Name) */

    ecs_fini(world);
}

void SkipUnknown_unknown_component_w_value(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownComp: {x: 10, y: 20}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup(world, "NsUnknownComp") == 0);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_w_nested_value(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownComp: {"
    LINE "    a: {b: 1, c: {d: 2}},"
    LINE "    e: [1, 2, 3]"
    LINE "  }"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup(world, "NsUnknownComp") == 0);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_in_scope(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "e {"
    LINE "  night_shift.NsWalkRectDecl: {w: 1}"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup(world, "night_shift") == 0);
    test_assert(ecs_lookup(world, "night_shift.NsWalkRectDecl") == 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 10);
    test_int(p->y, 20);

    ecs_fini(world);
}

void SkipUnknown_unknown_pair(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  (NsUnknownRel, NsUnknownTgt)"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup(world, "NsUnknownRel") == 0);
    test_assert(ecs_lookup(world, "NsUnknownTgt") == 0);

    ecs_fini(world);
}

void SkipUnknown_unknown_member_on_known_component(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "e {"
    LINE "  Position: {x: 10, unknown_member: 5, y: 20}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 10);
    test_int(p->y, 20);

    ecs_fini(world);
}

void SkipUnknown_unknown_nested_member_on_known_component(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "e {"
    LINE "  Position: {x: 10, unknown_member: {a: 1, b: {c: 2}}, y: 20}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 10);
    test_int(p->y, 20);

    ecs_fini(world);
}

void SkipUnknown_unknown_function_in_expr(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "a {"
    LINE "  Position: {x: wayfinding.blades(4), y: 20}"
    LINE "}"
    LINE "b {"
    LINE "  Position: {x: 1, y: 2}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Position) == NULL);

    ecs_entity_t b = ecs_lookup(world, "b");
    test_assert(b != 0);
    const Position *p = ecs_get(world, b, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 2);

    ecs_fini(world);
}

void SkipUnknown_unknown_vector_in_for(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "for v in $ns_unknown_vector {"
    LINE "  e {}"
    LINE "}"
    LINE "after {}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "e") == 0);
    test_assert(ecs_lookup(world, "after") != 0);

    ecs_fini(world);
}

void SkipUnknown_unknown_function_in_for(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "for v in wayfinding.points(4) {"
    LINE "  e {}"
    LINE "}"
    LINE "after {}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "e") == 0);
    test_assert(ecs_lookup(world, "after") != 0);

    ecs_fini(world);
}

void SkipUnknown_warn_once_per_name(void) {
    skip_unknown_warn_capture();

    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "a { NsUnknownTag }"
    LINE "b { NsUnknownTag }"
    LINE "c { NsUnknownTag }";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);
    test_int(skip_unknown_warn_count, 1);

    ecs_fini(world);

    skip_unknown_warn_release();
}

void SkipUnknown_warn_per_distinct_name(void) {
    skip_unknown_warn_capture();

    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "a { NsUnknownTagA }"
    LINE "b { NsUnknownTagB }"
    LINE "c { NsUnknownTagA }"
    LINE "d { NsUnknownTagB }";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);
    test_int(skip_unknown_warn_count, 2);

    ecs_fini(world);

    skip_unknown_warn_release();
}

void SkipUnknown_strict_unknown_tag_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownTag"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_strict_unknown_member_errors(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "e {"
    LINE "  Position: {x: 10, unknown_member: 5}"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_skip_unknown_disabled_after_enable(void) {
    ecs_world_t *world = ecs_init();

    test_assert(ecs_script_run_w_desc(world, NULL, "a { NsUnknownTag }", &skip_unknown_desc, NULL) == 0);

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, "b { NsUnknownTag }", &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_no_placeholder_then_strict_load(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "Position") == 0);

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 10);
    test_int(p->y, 20);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_component(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template Lamppost {"
    LINE "  prop height: f32 = 5"
    LINE "  Position: {x: 1, y: $height}"
    LINE "  Nightlight: {intensity: 3}"
    LINE "  HoloCycle"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  Lamppost: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "Nightlight") == 0);
    test_assert(ecs_lookup(world, "HoloCycle") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_template_child_w_unknown_component(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template Lamppost {"
    LINE "  prop height: f32 = 5"
    LINE "  Position: {x: 1, y: $height}"
    LINE "  head {"
    LINE "    Position: {x: 3, y: 4}"
    LINE "    Nightlight: {intensity: 3}"
    LINE "  }"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  Lamppost: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    ecs_entity_t head = ecs_lookup(world, "e.head");
    test_assert(head != 0);

    const Position *p = ecs_get(world, head, Position);
    test_assert(p != NULL);
    test_int(p->x, 3);
    test_int(p->y, 4);

    ecs_fini(world);
}

void SkipUnknown_isa_unresolved_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e : NsUnknownPrefab {"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_eval_desc(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownTag"
    LINE "}";

    ecs_script_eval_desc_t desc = { .ir = ir_enabled, .skip_unknown = true };
    ecs_script_t *script = ecs_script_parse(world, NULL, expr, &desc, NULL);
    test_assert(script != NULL);
    test_assert(ecs_script_eval(script, &desc, NULL) == 0);
    ecs_script_free(script);

    test_assert(ecs_lookup(world, "e") != 0);
    test_assert(ecs_lookup(world, "NsUnknownTag") == 0);

    ecs_fini(world);
}

void SkipUnknown_managed_script_desc(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownTag"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}";

    ecs_entity_t s = ecs_script(world, { .ir = ir_enabled,
        .code = expr,
        .skip_unknown = true
    });
    test_assert(s != 0);

    const EcsScript *sc = ecs_get(world, s, EcsScript);
    test_assert(sc != NULL);
    test_assert(sc->error == NULL);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup(world, "NsUnknownTag") == 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 10);
    test_int(p->y, 20);

    ecs_fini(world);
}

void SkipUnknown_managed_script_strict_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "e {"
    LINE "  NsUnknownTag"
    LINE "}";

    ecs_log_set_level(-4);
    ecs_entity_t s = ecs_script(world, { .ir = ir_enabled,
        .code = expr
    });
    ecs_log_set_level(-1);
    test_assert(s != 0);

    const EcsScript *sc = ecs_get(world, s, EcsScript);
    test_assert(sc != NULL);
    test_assert(sc->error != NULL);

    ecs_fini(world);
}

void SkipUnknown_unknown_with_tag(void) {
    ecs_world_t *world = ecs_init();

    ECS_TAG(world, Foo);

    const char *expr =
    HEAD "with NsUnknownTag, Foo {"
    LINE "  e {}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_has_id(world, e, Foo));
    test_assert(ecs_lookup(world, "NsUnknownTag") == 0);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_prop_type_w_default(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop gates: WallGates = []"
    LINE "  prop height: f32 = 5"
    LINE "  Position: {x: 1, y: $height}"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "WallGates") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_prop_type_no_default(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop gates: WallGates"
    LINE "  prop height: f32 = 5"
    LINE "  Position: {x: 1, y: $height}"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "WallGates") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_prop_type_in_for(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop gates: WallGates = []"
    LINE "  prop height: f32 = 5"
    LINE "  for g in gates {"
    LINE "    gate {"
    LINE "      Position: {x: g.x, y: g.y}"
    LINE "    }"
    LINE "  }"
    LINE "  Position: {x: 1, y: $height}"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "WallGates") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    test_assert(ecs_lookup_child(world, e, "gate") == 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_prop_type_in_expr(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop gates: WallGates = []"
    LINE "  prop height: f32 = 5"
    LINE "  child {"
    LINE "    Position: {x: $gates.count, y: 2}"
    LINE "  }"
    LINE "  Position: {x: 1, y: $height}"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "WallGates") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    ecs_entity_t child = ecs_lookup_child(world, e, "child");
    test_assert(child != 0);

    const Position *cp = ecs_get(world, child, Position);
    if (cp) {
        test_int(cp->x, 0);
        test_int(cp->y, 0);
    }

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_prop_type_set_at_instantiate(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop gates: WallGates = []"
    LINE "  prop height: f32 = 5"
    LINE "  Position: {x: 1, y: $height}"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {gates: [{x: 1}], height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "WallGates") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_const_type(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop height: f32 = 5"
    LINE "  const gates: WallGates = []"
    LINE "  Position: {x: 1, y: $height}"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {height: 7}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "WallGates") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 7);

    ecs_fini(world);
}

void SkipUnknown_const_w_unresolved_function_initializer(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "const gates = wayfinding.points(4)"
    LINE "e {"
    LINE "  Position: {x: 1, y: 2}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 2);

    ecs_fini(world);
}

void SkipUnknown_strict_unknown_prop_type_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "template CityWall {"
    LINE "  prop gates: WallGates = []"
    LINE "  prop height: f32 = 5"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  CityWall: {height: 7}"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_managed_script_skip_unknown_after_table_change(void) {
    ecs_world_t *world = ecs_init();

    ECS_TAG(world, Foo);

    ecs_entity_t s = ecs_script(world, { .ir = ir_enabled,
        .code = "e { NsUnknownTag }",
        .skip_unknown = true
    });
    test_assert(s != 0);

    const EcsScript *sc = ecs_get(world, s, EcsScript);
    test_assert(sc != NULL);
    test_bool(sc->skip_unknown, true);

    ecs_add(world, s, Foo);

    sc = ecs_get(world, s, EcsScript);
    test_assert(sc != NULL);
    test_bool(sc->skip_unknown, true);

    test_int(ecs_script_update(world, s, 0, "e2 { NsOtherUnknownTag }"), 0);

    sc = ecs_get(world, s, EcsScript);
    test_assert(sc != NULL);
    test_assert(sc->error == NULL);

    test_assert(ecs_lookup(world, "e2") != 0);

    ecs_fini(world);
}

typedef struct Label {
    char *text;
    int32_t size;
} Label;

static ecs_entity_t skip_unknown_label_component(
    ecs_world_t *world)
{
    return ecs_struct(world, {
        .entity = ecs_entity(world, {.name = "Label"}),
        .members = {
            {"text", ecs_id(ecs_string_t)},
            {"size", ecs_id(ecs_i32_t)}
        }
    });
}

void SkipUnknown_unknown_using(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "using ns_unknown_module"
    LINE "using ns_unknown_module.*"
    LINE "e {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "ns_unknown_module") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);
    const Position *p = ecs_get(world, e, Position);
    test_assert(p != NULL);
    test_int(p->x, 10);
    test_int(p->y, 20);

    ecs_fini(world);
}

void SkipUnknown_strict_unknown_using_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "using ns_unknown_module.*"
    LINE "e {}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_unknown_fn_param_type(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "fn sum(v: NsUnknownVec, k: i32) -> f32 {"
    LINE "  k"
    LINE "}"
    LINE "a {"
    LINE "  Position: {x: sum(1, 2), y: 20}"
    LINE "}"
    LINE "b {"
    LINE "  Position: {x: 1, y: 2}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsUnknownVec") == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Position) == NULL);

    ecs_entity_t b = ecs_lookup(world, "b");
    test_assert(b != 0);
    const Position *p = ecs_get(world, b, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 2);

    ecs_fini(world);
}

void SkipUnknown_unknown_fn_return_type(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    const char *expr =
    HEAD "fn make(k: i32) -> NsUnknownVec {"
    LINE "  k"
    LINE "}"
    LINE "a {"
    LINE "  Position: {x: 1, y: 2}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsUnknownVec") == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    const Position *p = ecs_get(world, a, Position);
    test_assert(p != NULL);
    test_int(p->x, 1);
    test_int(p->y, 2);

    ecs_fini(world);
}

void SkipUnknown_strict_unknown_fn_param_type_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "fn sum(v: NsUnknownVec, k: i32) -> f32 {"
    LINE "  k"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_expr(void) {
    ecs_world_t *world = ecs_init();

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "hud {"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  a {"
    LINE "    Label: {text: \"hour\", size: state.hour}"
    LINE "  }"
    LINE "  b {"
    LINE "    Label: {text: \"day\", size: 12}"
    LINE "  }"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsBkHud") == 0);

    ecs_entity_t a = ecs_lookup(world, "hud.a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "hud.b");
    test_assert(b != 0);
    const Label *l = ecs_get(world, b, Label);
    test_assert(l != NULL);
    test_str(l->text, "day");
    test_int(l->size, 12);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_interpolated_string(void) {
    ecs_world_t *world = ecs_init();

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "hud {"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  a {"
    LINE "    Label: {text: \"{state.hour_text}\", size: 12}"
    LINE "  }"
    LINE "  b {"
    LINE "    Label: {text: \"day\", size: 12}"
    LINE "  }"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsBkHud") == 0);

    ecs_entity_t a = ecs_lookup(world, "hud.a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "hud.b");
    test_assert(b != 0);
    const Label *l = ecs_get(world, b, Label);
    test_assert(l != NULL);
    test_str(l->text, "day");
    test_int(l->size, 12);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_binary_expr(void) {
    ecs_world_t *world = ecs_init();

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "hud {"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  a {"
    LINE "    Label: {text: \"hour\", size: state.hour + 1}"
    LINE "  }"
    LINE "  b {"
    LINE "    Label: {text: \"day\", size: 12}"
    LINE "  }"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t a = ecs_lookup(world, "hud.a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "hud.b");
    test_assert(b != 0);
    test_assert(ecs_get(world, b, Label) != NULL);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_fn_arg(void) {
    ecs_world_t *world = ecs_init();

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "fn twice(k: i32) -> i32 {"
    LINE "  k * 2"
    LINE "}"
    LINE "hud {"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  a {"
    LINE "    Label: {text: \"hour\", size: twice(state.hour)}"
    LINE "  }"
    LINE "  b {"
    LINE "    Label: {text: \"day\", size: twice(6)}"
    LINE "  }"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t a = ecs_lookup(world, "hud.a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "hud.b");
    test_assert(b != 0);
    const Label *l = ecs_get(world, b, Label);
    test_assert(l != NULL);
    test_int(l->size, 12);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_if(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "hud {"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  if state.on {"
    LINE "    a {}"
    LINE "  }"
    LINE "  b {}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "hud.a") == 0);
    test_assert(ecs_lookup(world, "hud.b") != 0);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_for(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "hud {"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  for v in state.items {"
    LINE "    a {}"
    LINE "  }"
    LINE "  b {}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "hud.a") == 0);
    test_assert(ecs_lookup(world, "hud.b") != 0);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_read_member_in_template(void) {
    ecs_world_t *world = ecs_init();

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "template Hud {"
    LINE "  prop size: i32 = 12"
    LINE "  const state = NsBkHud[NsBkHud]"
    LINE "  a {"
    LINE "    Label: {text: \"{state.hour_text}\", size: 12}"
    LINE "  }"
    LINE "  b {"
    LINE "    Label: {text: \"day\", size: size}"
    LINE "  }"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  Hud: {size: 14}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    ecs_entity_t a = ecs_lookup_child(world, e, "a");
    test_assert(a != 0);
    const Label *al = ecs_get(world, a, Label);
    if (al) {
        test_assert(al->text == NULL);
        test_int(al->size, 0);
    }

    ecs_entity_t b = ecs_lookup_child(world, e, "b");
    test_assert(b != 0);
    const Label *l = ecs_get(world, b, Label);
    test_assert(l != NULL);
    test_str(l->text, "day");
    test_int(l->size, 14);

    ecs_fini(world);
}

void SkipUnknown_template_w_unknown_prop_type_member_in_interpolated_string(void) {
    ecs_world_t *world = ecs_init();

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "template Sheet {"
    LINE "  prop wall: NsRgba = {172, 170, 164, 255}"
    LINE "  prop size: i32 = 12"
    LINE "  const svg = \"<rect fill='rgb({wall.r},{wall.g})'/>\""
    LINE "  a {"
    LINE "    Label: {text: svg, size: 1}"
    LINE "  }"
    LINE "  b {"
    LINE "    Label: {text: \"plain\", size: size}"
    LINE "  }"
    LINE "}"
    LINE ""
    LINE "e {"
    LINE "  Sheet: {size: 14}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsRgba") == 0);

    ecs_entity_t e = ecs_lookup(world, "e");
    test_assert(e != 0);

    ecs_entity_t a = ecs_lookup_child(world, e, "a");
    test_assert(a != 0);
    const Label *al = ecs_get(world, a, Label);
    if (al) {
        test_assert(al->text == NULL);
        test_int(al->size, 0);
    }

    ecs_entity_t b = ecs_lookup_child(world, e, "b");
    test_assert(b != 0);
    const Label *l = ecs_get(world, b, Label);
    test_assert(l != NULL);
    test_str(l->text, "plain");
    test_int(l->size, 14);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_on_known_entity_member(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "foo {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}"
    LINE "a {"
    LINE "  Label: {text: \"hi\", size: foo[NsUnknown].bar}"
    LINE "}"
    LINE "b {"
    LINE "  Label: {text: \"hi\", size: 12}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsUnknown") == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "b");
    test_assert(b != 0);
    test_assert(ecs_get(world, b, Label) != NULL);

    ecs_fini(world);
}

void SkipUnknown_unknown_component_on_known_entity_member_in_string(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "foo {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}"
    LINE "a {"
    LINE "  Label: {text: \"{foo[NsUnknown].bar}\", size: 12}"
    LINE "}"
    LINE "b {"
    LINE "  Label: {text: \"hi\", size: 12}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    test_assert(ecs_lookup(world, "NsUnknown") == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "b");
    test_assert(b != 0);
    test_assert(ecs_get(world, b, Label) != NULL);

    ecs_fini(world);
}

void SkipUnknown_unknown_member_on_known_component_read(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "foo {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}"
    LINE "a {"
    LINE "  Label: {text: \"hi\", size: foo[Position].nope}"
    LINE "}"
    LINE "b {"
    LINE "  Label: {text: \"hi\", size: 12}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "b");
    test_assert(b != 0);
    test_assert(ecs_get(world, b, Label) != NULL);

    ecs_fini(world);
}

void SkipUnknown_unknown_member_on_known_component_read_in_string(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "foo {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}"
    LINE "a {"
    LINE "  Label: {text: \"{foo[Position].nope}\", size: 12}"
    LINE "}"
    LINE "b {"
    LINE "  Label: {text: \"hi\", size: 12}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    test_assert(ecs_get(world, a, Label) == NULL);

    ecs_entity_t b = ecs_lookup(world, "b");
    test_assert(b != 0);
    test_assert(ecs_get(world, b, Label) != NULL);

    ecs_fini(world);
}

void SkipUnknown_known_component_read_member_in_string(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);

    const char *expr =
    HEAD "foo {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "  Label: {text: \"foo\", size: 7}"
    LINE "}"
    LINE "a {"
    LINE "  Label: {text: \"size={foo[Label].size}\", size: 12}"
    LINE "}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);

    ecs_entity_t a = ecs_lookup(world, "a");
    test_assert(a != 0);
    const Label *l = ecs_get(world, a, Label);
    test_assert(l != NULL);
    test_str(l->text, "size=7");

    ecs_fini(world);
}

void SkipUnknown_strict_unknown_member_on_known_component_read_errors(void) {
    ecs_world_t *world = ecs_init();

    ECS_COMPONENT(world, Position);
    ecs_struct(world, {
        .entity = ecs_id(Position),
        .members = {
            {"x", ecs_id(ecs_f32_t)},
            {"y", ecs_id(ecs_f32_t)}
        }
    });

    ecs_entity_t ecs_id(Label) = skip_unknown_label_component(world);
    (void)ecs_id(Label);

    const char *expr =
    HEAD "foo {"
    LINE "  Position: {x: 10, y: 20}"
    LINE "}"
    LINE "a {"
    LINE "  Label: {text: \"hi\", size: foo[Position].nope}"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &ir_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_template_unresolved_base_errors(void) {
    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "template Panel : NsUnknownBase {"
    LINE "  prop w: f32 = 3"
    LINE "}";

    ecs_log_set_level(-4);
    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) != 0);
    ecs_log_set_level(-1);

    ecs_fini(world);
}

void SkipUnknown_warn_once_for_unknown_using(void) {
    skip_unknown_warn_capture();

    ecs_world_t *world = ecs_init();

    const char *expr =
    HEAD "using ns_unknown_module.*"
    LINE "using ns_unknown_module.*"
    LINE "e {}"
    LINE "f {}";

    test_assert(ecs_script_run_w_desc(world, NULL, expr, &skip_unknown_desc, NULL) == 0);
    test_int(skip_unknown_warn_count, 1);

    ecs_fini(world);

    skip_unknown_warn_release();
}
