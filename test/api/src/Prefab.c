...
void Iter(ecs_view_t *view) {
    ...
    bool shared = ecs_is_shared(view, 1);
...
void Prefab_new_w_count_prefab() {
    ...
    for (i = e_start; i < (e_start + 10); i ++) {
        const Position *p = ecs_get_ptr(world, i, Position);
        test_assert(p != NULL);
...
void Prefab_new_w_type_w_prefab() {
    ...
    const Position *p_1 = ecs_get_ptr(world, e_1, Position);
    const Position *p_2 = ecs_get_ptr(world, e_2, Position);
    const Position *p_prefab = ecs_get_ptr(world, Prefab, Position);
...
void Prefab_override_2_of_3_components_1_self() {
    ...
    const Velocity *v_prefab = ecs_get_ptr(world, Prefab, Velocity);
    ...
    const Mass *m_prefab = ecs_get_ptr(world, Prefab, Mass);
...
void Prefab_get_ptr_prefab() {
    ...
    test_assert(e_1 != 0);
    test_assert( ecs_has_entity(world, e_1, ECS_INSTANCEOF | Prefab));
    test_assert( ecs_get_ptr_w_entity(world, e_1, Prefab) == NULL);

    ecs_fini(world);
}

static
void Prefab_w_field(ecs_view_t *view) {
    probe_system(view);
...
static
void Prefab_w_shared(ecs_view_t *view) {
    Velocity *v = NULL;
...
void Prefab_new_w_count_w_override() {
    ...
    const Position *prefab_p = ecs_get_ptr(world, Prefab, Position);
    const Velocity *prefab_v = ecs_get_ptr(world, Prefab, Velocity);
...
void Prefab_override_2_components_different_size() {
    ...
    const Position *prefab_p = ecs_get_ptr(world, Prefab, Position);
    const Velocity *prefab_v = ecs_get_ptr(world, Prefab, Velocity);
    const Color *prefab_c = ecs_get_ptr(world, Prefab, Color);
    ...
    for (e = e_1; e < (e_1 + 100); e ++) {
        ...
        const Color *c = ecs_get_ptr(world, e, Color);
        test_assert(c != NULL);
        test_int(c->r, 1);
        test_int(c->g, 2);
        test_int(c->b, 3);
...
void Prefab_prefab_w_1_child() {
    ...
    const Position *p_e = ecs_get_ptr(world, e, Position);
    ...
    const Position *p_child = ecs_get_ptr(world, e_child, Position);
...
void Prefab_prefab_w_base_w_child() {
    ...
    const Velocity *v_e = ecs_get_ptr(world, e, Velocity);
...
void Prefab_prefab_w_child_w_base() {
    ...
    const Velocity *v_child = ecs_get_ptr(world, e_child, Velocity);
...
void Prefab_prefab_w_child_w_base_w_children() {
    ...
    const Position *p_base_child = ecs_get_ptr(world, e_base_child, Position);
...
void Prefab_prefab_auto_override_child_component() {
    ...
    const Position *p1 = ecs_get_ptr(world, e1, Position);
    test_assert(p1 != NULL);
    test_int(p1->x, 1);
    test_int(p1->y, 2);

    const Position *p2 = ecs_get_ptr(world, e2, Position);
    test_assert(p2 != NULL);
    test_int(p2->x, 1);
    test_int(p2->y, 2);    
    test_assert(p1 == p2);
    ...
    const Position *child_p1 = ecs_get_ptr(world, child_1, Position);
    test_assert(child_p1 != NULL);
    test_int(child_p1->x, 2);
    test_int(child_p1->y, 3);

    const Position *child_p2 = ecs_get_ptr(world, child_2, Position);
    test_assert(child_p2 != NULL);
    test_int(child_p2->x, 2);
    test_int(child_p2->y, 3);   
    test_assert(child_p1 == child_p2); 

    const Velocity *child_v1 = ecs_get_ptr(world, child_1, Velocity);
    test_assert(child_v1 != NULL);
    test_int(child_v1->x, 4);
    test_int(child_v1->y, 5);

    const Velocity *child_v2 = ecs_get_ptr(world, child_2, Velocity);
...
void PrefabReactiveTest(ecs_view_t *view) {
    invoked ++;
}
...
void InstantiateInProgress(ecs_view_t *view) {
    ECS_COLUMN_ENTITY(view, Prefab, 2);

    ecs_entity_t *ids = ecs_get_context(view->world);

    int i;
    for (i = 0; i < view->count; i ++) {
        ids[i] = ecs_new_w_entity(view->world, ECS_INSTANCEOF | Prefab);
    }
}
...
void Prefab_instantiate_in_progress() {
    ...
    int i;
    for (i = 0; i < 10; i ++) {
        const Velocity *v = ecs_get_ptr(world, ids[i], Velocity);
        test_assert(v == v_prefab);
    }
...
void NewInProgress(ecs_view_t *view) {
    ECS_COLUMN_COMPONENT(view, Type, 2);

    ecs_entity_t *ids = ecs_get_context(view->world);

    int i;
    for (i = 0; i < view->count; i ++) {
        ids[i] = ecs_new(view->world, Type);
    }
}
...
void AddPrefab(ecs_view_t *view) {
    ECS_COLUMN_ENTITY(view, Prefab, 2);

    int i;
    for (i = 0; i < view->count; i ++) {
        ecs_add_entity(view->world, view->entities[i], ECS_INSTANCEOF | Prefab);
    }
}
...
void Prefab_no_instantiate_on_2nd_add() {
    ...
    test_assert(e_child != 0);
    const Velocity *v = ecs_get_ptr(world, e_child, Velocity);
    test_assert(v != NULL);
    test_int(v->x, 3);
    test_int(v->y, 4);
...
void NewPrefab_w_count(ecs_view_t *view) {
    ecs_entity_t *result = ecs_get_context(view->world);
    ECS_COLUMN_ENTITY(view, Prefab, 1);
    *result = ecs_bulk_new_w_entity(view->world, ECS_INSTANCEOF | Prefab, 3);
}
...
void Prefab_nested_prefab_in_progress_w_count() {
    ...
    for (i = 0; i < 3; i ++) {
        ...
        const Position *p = ecs_get_ptr(world, result + i, Position);
        test_assert(p != NULL); 
        test_int(p->x, 1);
        test_int(p->y, 2);
        ...
        const Velocity *v = ecs_get_ptr(world, child, Velocity);
        test_assert(v != NULL);
        test_int(v->x, 3);
        test_int(v->y, 4);
    }
...
static
void OnSetVelocity(ecs_view_t *view) {
    ...
    for (i = 0; i < view->count; i ++) {
        ...
        Velocity *staged_v = ecs_get_mut(view->world, view->entities[i], Velocity, NULL);
        test_assert(ecs_get_ptr(view->world, view->entities[i], Velocity) == staged_v);
...
void AddPrefabInProgress(ecs_view_t *view) {
    ECS_COLUMN_COMPONENT(view, Prefab, 2);
    ECS_COLUMN_COMPONENT(view, Velocity, 3);

    ecs_entity_t Prefab = ecs_type_to_entity(view->world, ecs_type(Prefab));

    int i;
    for (i = 0; i < view->count; i ++) {
        ecs_add_entity(view->world, ECS_INSTANCEOF | view->entities[i], Prefab);
        test_assert( ecs_has(view->world, view->entities[i], Prefab));
        test_assert( ecs_has(view->world, view->entities[i], Velocity));
        
        const Velocity *v = ecs_get_ptr(view->world, view->entities[i], Velocity);
        test_assert(v != NULL);
        test_int(v->x, 1);
        test_int(v->y, 2);

        test_assert( ecs_get_ptr(view->world, Prefab, Velocity) == v);
    }
}
...
void AddMass(ecs_view_t *view) {
    ECS_COLUMN_ENTITY(view, Mass, 2);

    int i;
    for (i = 0; i < view->count; i ++) {
        ecs_add(view->world, view->entities[i], Mass);
    }
}
...
void Prefab_override_watched_prefab() {
    ...
    const Position *p1 = ecs_get_ptr(world, Prefab, Position);
    const Position *p2 = ecs_get_ptr(world, Entity1, Position);
    const Position *p3 = ecs_get_ptr(world, Entity2, Position);
...
static
void Inherit(ecs_view_t *view) {
    ECS_COLUMN_COMPONENT(view, Position, 1);

    ecs_entity_t *param = ecs_get_context(view->world);

    int i;
    for (i = 0; i < view->count; i ++) {
        ecs_entity_t e = view->entities[i];
        ecs_entity_t e_backup = ecs_new(view->world, Position);
        test_assert(e_backup != 0);
        test_assert( ecs_has(view->world, e_backup, Position));

        ecs_add_entity(view->world, e, ECS_INSTANCEOF | e_backup);
        test_assert( ecs_has_entity(view->world, e, e_backup | ECS_INSTANCEOF));
...
void AddPosition(ecs_view_t *view) {
    ...
    ecs_entity_t *base = ecs_get_context(view->world);

    ecs_add(view->world, *base, Position);
}
...
static
void CloneInOnAdd(ecs_view_t *view)
{
    ECS_COLUMN(view, Position, p, 1);
    ...
    for (i = 0; i < view->count; i ++) {
        ...
        if (!has_cloned_test) {
            ecs_entity_t e = view->entities[i];
            ecs_entity_t clone = ecs_clone(view->world, 0, e, true);
            test_assert( ecs_has(view->world, clone, Position));

            const Position *p_clone = ecs_get_ptr(view->world, clone, Position);
            test_int(p_clone->x, 10);
            test_int(p_clone->y, 20);

            ecs_add_entity(view->world, ECS_INSTANCEOF | clone, e);
        }
    }
}
...
static
void OnAddEntity(ecs_view_t *view) {
    ECS_COLUMN(view, ecs_entity_t, e, 1);
...
void Prefab_create_multiple_nested_w_on_set() {
    ...
    const ecs_entity_t *self1 = ecs_get_ptr(world, child1, ecs_entity_t);
    test_assert(self1 != NULL);
    test_assert(*self1 == child1);

    const ecs_entity_t *self2 = ecs_get_ptr(world, child2, ecs_entity_t);
    ...
    const Velocity *v1 = ecs_get_ptr(world, child1, Velocity);
    test_assert(v1 != NULL);
    test_int(v1->x, 4);
    test_int(v1->y, 5);

    const Velocity *v2 = ecs_get_ptr(world, child2, Velocity);
    test_assert(v2 != NULL);
    test_assert(v1 != v2);
    test_int(v2->x, 4);
    test_int(v2->y, 5);
...
static
void CreateInstances(ecs_view_t *view) {
    ECS_COLUMN_ENTITY(view, Prefab, 1);

    new_instance_1 = ecs_new_w_entity(view->world, ECS_INSTANCEOF | Prefab);
    new_instance_2 = ecs_new_w_entity(view->world, ECS_INSTANCEOF | Prefab);
}
