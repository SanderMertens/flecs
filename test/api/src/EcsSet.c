...
void SetExisting(EcsRows *rows) {
    ...
    EcsEntity int8_t_h = ctx->handle;
    EcsWorld *world = rows->world;

    void *row;
    for (row = rows->first; row < rows->last; row = ecs_next(rows, row)) {
        int8_t value = ecs_get(world, ecs_entity(rows, row, ECS_ROW_ENTITY), int8_t);
        ecs_set(world, ecs_entity(rows, row, ECS_ROW_ENTITY), int8_t, {value + 1});
    }
}
    ...
    test_EcsSet this)
{
    ...
    EcsEntity e1 = ecs_new(world, int8_t_h);
    EcsEntity e2 = ecs_new(world, int8_t_h);
    test_assert(e1 != 0);
    test_assert(e2 != 0);
...
void Set(EcsRows *rows) {
    Context *ctx = ecs_get_context(rows->world);
    EcsEntity int16_t_h = ctx->handle;
    EcsWorld *world = rows->world;
    int16_t value = 10;

    void *row;
    for (row = rows->first; row < rows->last; row = ecs_next(rows, row)) {
        ecs_set(world, ecs_entity(rows, row, ECS_ROW_ENTITY), int16_t, {value});
        value += 10;
    }
}
...
void Set2(EcsRows *rows) {
    Context *ctx = ecs_get_context(rows->world);
    EcsEntity int16_t_h = ctx->handle;
    EcsEntity int32_t_h = ctx->handle2;
    EcsWorld *world = rows->world;
    int16_t value1 = 10;
    int32_t value2 = 1;
    ...
    void *row;
    for (row = rows->first; row < rows->last; row = ecs_next(rows, row)) {
        ecs_set(world, ecs_entity(rows, row, ECS_ROW_ENTITY), int16_t, {value1});
        ecs_set(world, ecs_entity(rows, row, ECS_ROW_ENTITY), int32_t, {value2});
        value1 += 10;
        value2 += 1;
    }
}
...
void Set2OnNew(EcsRows *rows) {
    Context *ctx = ecs_get_context(rows->world);
    EcsEntity int8_t_h = ctx->handle;
    EcsEntity int16_t_h = ctx->handle2;
    EcsWorld *world = rows->world;

    void *row;
    for (row = rows->first; row < rows->last; row = ecs_next(rows, row)) {
        EcsEntity e = ecs_new(world, int8_t_h);
    ...
    test_EcsSet this)
{
    ...
    EcsEntity e1 = ecs_new(world, float_h);
    test_assert(e1 != 0);
    ...
    test_EcsSet this)
{
    ...
    EcsEntity e2 = ecs_new(world, int8_16_h);
    test_assert(e1 != 0);
    test_assert(e2 != 0);
...
void SetNew(EcsRows *rows) {
    ...
    for (row = rows->first; row < rows->last; row = ecs_next(rows, row)) {
        EcsEntity e = ecs_new(world, int8_t_h);
        int8_t value = *(int8_t*)ecs_data(rows, row, 0);
