...
void Observer_on_remove_value_pair_w_var_second_term(void) {
    ...
    ECS_COMPONENT(world, Position);
    ecs_add_id(world, ecs_id(Position), EcsRelationship);
    ecs_add_id(world, ecs_id(Position), EcsTrait);

    ECS_TAG(world, Trait);
    ecs_add_id(world, Trait, EcsTrait);

    ECS_TAG(world, Rel);
    ecs_add_id(world, Rel, EcsRelationship);
    ecs_add_id(world, Rel, Trait);

    ECS_TAG(world, ValRel);
    ecs_add_id(world, ValRel, EcsRelationship);

    Probe ctx = {0};
    ecs_observer(world, {
        .query.terms = {
            { .first.id = ecs_id(Position),
              .second.name = "TrackedRel", .second.id = EcsIsVariable,
              .src.name = "Targetee", .src.id = EcsIsVariable,
              .inout = EcsInOutFilter },
            { .first.name = "TrackedRel", .first.id = EcsIsVariable,
              .second.name = "Targetee", .second.id = EcsIsVariable },
            { .first.id = Trait,
              .src.name = "TrackedRel", .src.id = EcsIsVariable }
        },
        .events = { EcsOnRemove },
        .callback = Observer,
        .ctx = &ctx
    ...
    ecs_entity_t e = ecs_new(world);
    ecs_add_id(world, e, ecs_value_pair(ValRel, 0));
    test_assert(ecs_has_id(world, e, ecs_value_pair(ValRel, 0)));
    test_int(ctx.invoked, 0);
