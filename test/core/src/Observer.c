...
static int is_size_observer_invoked = 0;
...
static void Observer_w_size(ecs_iter_t *it) {
    ...
    if (it->field_count == 2) {
        size_observer_field_2 = ecs_field_size(it, 1);
    }

    is_size_observer_invoked ++;
}
...
void Observer_on_add_after_edge(void) {
    ...
    ecs_add(world, e1, TagA); // create edge from [] -> [TagA]
...
void Observer_on_add_wildcard_T_before_edge(void) {
    ...
    Probe ctx = {0};
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {{ ecs_pair(EcsWildcard, Tgt) }},
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    });
    ...
    test_null(ctx.param);
    test_int(ctx.e[0], e);
    test_int(ctx.c[0][0], ecs_pair(RelA, Tgt));
    ...
    test_null(ctx.param);
    test_int(ctx.e[0], e);
    test_int(ctx.c[0][0], ecs_pair(RelB, Tgt));
...
void Observer_on_add_wildcard_wildcard_before_edge(void) {
    ...
    Probe ctx = {0};
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {{ ecs_pair(EcsWildcard, EcsWildcard) }},
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    });
...
void Observer_on_remove_after_edge(void) {
    ...
    ecs_remove(world, e1, TagA); // create edge from [TagA] -> []
...
void Observer_on_remove_wildcard_T_before_edge(void) {
    ...
    ecs_remove_pair(world, e, RelA, Tgt);
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnRemove);
    ...
    ecs_remove_pair(world, e, RelB, Tgt);
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnRemove);
...
void Observer_on_remove_wildcard_T_after_edge(void) {
    ...
    ecs_add_pair(world, e1, RelA, Tgt);
    ecs_add_pair(world, e1, RelB, Tgt);
    ecs_remove_pair(world, e1, RelA, Tgt);
    ecs_remove_pair(world, e1, RelB, Tgt);
    ...
    ecs_remove_pair(world, e2, RelA, Tgt);
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnRemove);
    ...
    ecs_remove_pair(world, e2, RelB, Tgt);
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnRemove);
...
void Observer_on_add_optional(void) {
    ...
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {
            { .id = ecs_id(Position), .oper = EcsOptional }
        },
        .callback = Observer,
        .events = { EcsOnAdd },
        .ctx = &ctx
...
static int is_set_invoked = 0;

void IsSetObserver(ecs_iter_t *it) {
    test_assert(it->field_count > 0);
    for (int i = 0; i < it->field_count; i ++) {
        test_assert(ecs_field_is_set(it, i));
        is_set_invoked ++;
    }
}
...
void Observer_1_term_on_add_is_set(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ Foo }},
        .events = { EcsOnAdd },
        .callback = IsSetObserver
    });

    ecs_new_w(world, Foo);

    test_int(is_set_invoked, 1);
...
void Observer_2_terms_on_add_yield_existing_is_set(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Foo) }, { ecs_id(Bar) }},
        .events = { EcsOnAdd },
        .callback = IsSetObserver,
        .yield_existing = true
    });
...
void Observer_1_term_field_size(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position) }},
        .events = { EcsOnSet },
        .callback = Observer_w_size
    });

    test_int(is_size_observer_invoked, 0);

    ecs_entity_t e = ecs_new(world);
    ecs_set(world, e, Position, {10, 20});

    test_int(is_size_observer_invoked, 1);
    test_int(size_observer_field_1, sizeof(Position));
...
void Observer_2_terms_field_size(void) {
    ...
    test_int(is_size_observer_invoked, 1);
    test_int(size_observer_field_1, sizeof(Position));
    test_int(size_observer_field_2, sizeof(Velocity));
...
void Observer_1_up_term_field_size(void) {
    ...
    ecs_new_w_pair(world, EcsChildOf, p);
    test_int(is_size_observer_invoked, 0);
...
void Observer_2_up_terms_field_size(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = EcsUp }, { ecs_id(Velocity), .src.id = EcsUp }},
        .events = { EcsOnSet },
        .callback = Observer_w_size
    });
...
void Observer_1_fixed_src_term_field_size(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = ecs_id(Position) }},
        .events = { EcsOnSet },
        .callback = Observer_w_size
    });
...
void Observer_2_fixed_src_terms_field_size(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = ecs_id(Position) }, { ecs_id(Velocity), .src.id = ecs_id(Velocity) }},
        .events = { EcsOnSet },
        .callback = Observer_w_size
    });
...
void Observer_2_singleton_terms_field_size(void) {
    ...
    test_int(is_size_observer_invoked, 1);
    test_int(size_observer_field_1, sizeof(Position));
    test_int(size_observer_field_1, sizeof(Velocity));

    ecs_fini(world);
}

int is_on_set_position_invoked = 0;
...
void Observer_2_on_set_overridden_terms_field_size(void) {
    ...
    ecs_observer(world, {
        .query.terms =  {{ ecs_id(Position) }, { ecs_id(Mass) }},
        .events = { EcsOnSet },
        .callback = on_set_position_mass
    });

    ecs_new_w_pair(world, EcsIsA, p);
    test_int(is_on_set_position_mass_invoked, 1);
...
void Observer_1_on_set_after_remove_overridden_term_field_size(void) {
    ...
    ecs_entity_t p = ecs_new(world);
    ecs_set_override(world, p, Position, {10, 20});
    ecs_set_override(world, p, Mass, {1});
...
void Observer_2_on_set_after_remove_overridden_terms_field_size(void) {
    ...
    ecs_remove(world, e, Mass);
    test_int(is_on_set_position_mass_invoked, 2);
...
int is_on_add_position_invoked = 0;
...
void Observer_1_term_field_size_w_tag(void) {
    ...
    test_int(is_size_observer_invoked, 1);
    test_int(size_observer_field_1, 0);
...
void Observer_2_terms_field_size_w_tag(void) {
    ...
    test_int(is_size_observer_invoked, 1);
    test_int(size_observer_field_1, sizeof(Position));
    test_int(size_observer_field_2, 0);
...
void Observer_2_up_terms_field_size_w_tag(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = EcsUp }, { Foo, .src.id = EcsUp }},
        .events = { EcsOnSet },
        .callback = Observer_w_size
    });
    ...
    ecs_new_w_pair(world, EcsChildOf, p);
    test_int(is_size_observer_invoked, 1); // invoked because Foo term reacts to OnAdd
    test_int(size_observer_field_1, sizeof(Position));
    test_int(size_observer_field_2, 0);
...
void Observer_2_fixed_src_terms_field_size_w_tag(void) {
    ...
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = ecs_id(Position) }, { Foo, .src.id = Foo }},
        .events = { EcsOnSet },
        .callback = Observer_w_size
    });
...
void Observer_1_on_set_overridden_term_field_size_w_tag(void) {
    ...
    ecs_new_w_pair(world, EcsIsA, p);
    test_int(is_on_add_position_invoked, 1);
    test_int(is_on_add_foo_invoked, 1);
...
void Observer_2_on_set_overridden_terms_field_size_w_tag(void) {
    ...
    ecs_observer(world, {
        .query.terms =  {{ ecs_id(Position) }, { Foo }},
        .events = { EcsOnAdd },
        .callback = on_add_position_foo
    });

    ecs_new_w_pair(world, EcsIsA, p);
    test_int(is_on_add_position_foo_invoked, 1);
...
void Observer_on_add_childof_wildcard_w_parent_component(void) {
    ...
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {{ ecs_childof(EcsWildcard) }},
        .events = { EcsOnAdd },
        .callback = Observer,
        .ctx = &ctx
    });
...
void Observer_on_add_childof_any_w_parent_component(void) {
    ...
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {{ ecs_childof(EcsAny) }},
        .events = { EcsOnAdd },
        .callback = Observer,
        .ctx = &ctx
    });
    ...
    test_uint(ctx.e[0], e);
    test_uint(ctx.s[0][0], 0);
    // test_uint(ctx.c[0][0], ecs_childof(EcsWildcard)); TODO
...
void Observer_on_add_childof_w_parent_component_multi_term(void) {
    ...
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {{ ecs_childof(parent_a) }, { Foo }},
        .events = { EcsOnAdd },
        .callback = Observer,
        .ctx = &ctx
    });
...
void Observer_on_add_childof_any_w_parent_component_multi_term(void) {
    ...
    ecs_entity_t o = ecs_observer(world, {
        .query.terms = {{ ecs_childof(EcsAny) }, { Foo }},
        .events = { EcsOnAdd },
        .callback = Observer,
        .ctx = &ctx
    });
    ...
    test_uint(ctx.e[0], e);
    test_uint(ctx.s[0][0], 0);
    // test_uint(ctx.c[0][0], ecs_childof(EcsWildcard));
    test_uint(ctx.c[0][1], Foo);
...
void Observer_up_forward_w_parent_component_2_levels(void) {
    ecs_world_t *world = ecs_mini();
    ...
    ecs_entity_t root = ecs_insert(world, ecs_value(Position, {10, 20}));
    test_int(ctx.invoked, 0);

    ecs_entity_t parent = ecs_insert(world, ecs_value(EcsParent, {root}));
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnAdd);
    test_uint(ctx.e[0], parent);
    test_uint(ctx.s[0][0], root);
...
void Observer_up_forward_w_parent_component_2_levels_reverse(void) {
    ecs_world_t *world = ecs_mini();
    ...
    test_uint(ctx.s[0][0], root);
    test_uint(ctx.e[1], e);
    test_uint(ctx.s[1][0], root);

    ecs_fini(world);
}

void Observer_up_propagate_w_parent_component_2_levels(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_up_forward_w_parent_component_2_levels_mixed_1(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_up_forward_w_parent_component_2_levels_mixed_2(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_up_propagate_w_parent_component_2_levels_mixed_1(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_up_propagate_w_parent_component_2_levels_mixed_2(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_self_up_forward_w_parent_component_2_levels(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_self_up_forward_w_parent_component_2_levels_reverse(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_self_up_propagate_w_parent_component_2_levels(void) {
    ecs_world_t *world = ecs_mini();
    ...
    test_int(ctx.event, EcsOnAdd);
    test_uint(ctx.e[0], root);
    test_uint(ctx.e[1], parent);
    test_uint(ctx.e[2], e);
    test_uint(ctx.s[0][0], 0);
    test_uint(ctx.s[1][0], root);
    test_uint(ctx.s[2][0], root);

    ecs_fini(world);
}

void Observer_self_up_forward_w_parent_component_2_levels_mixed_1(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_self_up_forward_w_parent_component_2_levels_mixed_2(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_self_up_propagate_w_parent_component_2_levels_mixed_1(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_self_up_propagate_w_parent_component_2_levels_mixed_2(void) {
    ecs_world_t *world = ecs_mini();
...
void Observer_up_forward_w_parent_component_reparent(void) {
    ...
    ecs_entity_t parent_a = ecs_insert(world, ecs_value(Position, {10, 20}));
    test_int(ctx.invoked, 0);

    ecs_entity_t parent_b = ecs_insert(world, ecs_value(Position, {20, 30}));
    test_int(ctx.invoked, 0);

    ecs_entity_t e = ecs_insert(world, ecs_value(EcsParent, {parent_a}));
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnAdd);
    test_uint(ctx.e[0], e);
    test_uint(ctx.s[0][0], parent_a);
...
void Observer_self_up_forward_w_parent_component_reparent(void) {
    ...
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnAdd);
    test_uint(ctx.e[0], parent_a);
    test_uint(ctx.s[0][0], 0);

    ecs_os_zeromem(&ctx);

    ecs_entity_t parent_b = ecs_insert(world, ecs_value(Position, {10, 20}));
    test_int(ctx.invoked, 1);
    test_int(ctx.count, 1);
    test_int(ctx.system, o);
    test_int(ctx.event, EcsOnAdd);
    test_uint(ctx.e[0], parent_b);
    test_uint(ctx.s[0][0], 0);
...
static void Observer_propagate_on_set(ecs_iter_t *it) {
    ...
    Position *p = ecs_field(it, Position, 0);
    test_assert(p != NULL);
    test_assert(propagate_on_set_count < 2);
    propagate_on_set_values[propagate_on_set_count ++] = p[0];
}
...
void Observer_propagate_on_set_2_lvls(void) {
    ...
    ecs_entity_t root = ecs_insert(world, ecs_value(Position, {10, 20}));
    ecs_entity_t middle = ecs_insert(world, ecs_value(Position, {20, 30}));
    ecs_add_pair(world, middle, EcsChildOf, root);
    ecs_entity_t child = ecs_new_w_pair(world, EcsChildOf, middle);

    Probe ctx = {0};
    propagate_on_set_count = 0;
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = EcsUp }},
        .events = {EcsOnSet},
        .callback = Observer_propagate_on_set,
        .ctx = &ctx
    });

    ecs_set(world, root, Position, {30, 40});

    test_int(ctx.invoked, 1);
    test_uint(ctx.e[0], middle);
    test_uint(ctx.s[0][0], root);

    test_int(propagate_on_set_count, 1);
    test_int(propagate_on_set_values[0].x, 30);
    test_int(propagate_on_set_values[0].y, 40);
...
void Observer_propagate_on_set_2_lvls_2_terms(void) {
    ...
    propagate_on_set_count = 0;
    ecs_observer(world, {
        .query.terms = {{ ecs_id(Position), .src.id = EcsUp }, { Foo }},
        .events = {EcsOnSet},
        .callback = Observer_propagate_on_set,
        .ctx = &ctx
    });
...
typedef struct observer_event_log_t {
    int32_t count;
    ecs_entity_t events[16];
    ecs_id_t event_ids[16];
    ecs_entity_t sources[16];
    ecs_entity_t entities[16];
} observer_event_log_t;

static void ObserverLogEvents(ecs_iter_t *it) {
    observer_event_log_t *log = it->ctx;
    int32_t i;
    for (i = 0; i < it->count; i ++) {
        test_assert(log->count < 16);
        log->events[log->count] = it->event;
        log->event_ids[log->count] = it->event_id;
        log->sources[log->count] = ecs_field_src(it, 0);
        log->entities[log->count] = it->entities[i];
        log->count ++;
    }
}
...
void Observer_propagate_masked_add_to_grandparent(void) {
    ...
    ecs_add_id(world, c, ecs_new(world));

    Probe ctx = {0};
    ecs_observer(world, {
        .query.terms = {{ Tag, .src.id = EcsUp }},
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    });

    ecs_add(world, gp, Tag);
    test_int(ctx.invoked, 1);
    test_uint(ctx.e[0], p);
    test_uint(ctx.s[0][0], gp);
...
void Observer_propagate_self_up_add_to_parent_w_child_owned(void) {
    ...
    Probe ctx = {0};
    ecs_observer(world, {
        .query.terms = {{ Tag, .src.id = EcsSelf|EcsUp }},
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    });
...
void Observer_propagate_remove_from_parent_w_grandparent_owned(void) {
    ...
    observer_event_log_t log = {0};
    ecs_observer(world, {
        .query.terms = {{ Tag, .src.id = EcsUp }},
        .events = {EcsOnAdd, EcsOnRemove},
        .callback = ObserverLogEvents,
        .ctx = &log
    });

    ecs_remove(world, p, Tag);
    test_int(log.count, 1);
    test_uint(log.events[0], EcsOnRemove);
    test_uint(log.entities[0], c);
    test_uint(log.sources[0], p);
...
void Observer_propagate_add_to_parent_w_grandparent_owned(void) {
    ...
    ecs_add(world, p, Tag);
    test_int(log.count, 1);
    test_uint(log.events[0], EcsOnAdd);
    test_uint(log.entities[0], c);
    test_uint(log.sources[0], p);
...
void Observer_propagate_add_to_parent_w_subtree(void) {
    ...
    ecs_add(world, p, Tag);
    test_int(ctx.invoked, 3);
    test_uint(ctx.e[0], c1);
    test_uint(ctx.e[1], c2);
    test_uint(ctx.e[2], c3);
    test_uint(ctx.s[0][0], p);
    test_uint(ctx.s[1][0], p);
    test_uint(ctx.s[2][0], p);
...
void Observer_propagate_add_stops_at_owned_table(void) {
    ...
    ecs_entity_t p = ecs_new(world);
    ecs_entity_t c1 = ecs_new_w_pair(world, EcsChildOf, p);
    ecs_add(world, c1, Tag);
    ecs_entity_t c2 = ecs_new_w_pair(world, EcsChildOf, c1);
    ecs_add_id(world, c2, ecs_new(world));
...
void Observer_propagate_reparent_w_both_parents_owned(void) {
    ...
    ecs_entity_t p1 = ecs_new(world);
    ecs_add(world, p1, Tag);
    ecs_entity_t p2 = ecs_new(world);
    ecs_add(world, p2, Tag);
    ecs_entity_t c = ecs_new_w_pair(world, EcsChildOf, p1);
    ...
    test_int(log.count, 2);
    test_uint(log.events[0], EcsOnRemove);
    test_uint(log.entities[0], c);
    test_uint(log.sources[0], p1);
    test_uint(log.events[1], EcsOnAdd);
    test_uint(log.entities[1], c);
    test_uint(log.sources[1], p2);
...
void Observer_propagate_reparent_parent_w_subtree(void) {
    ...
    ecs_entity_t r1 = ecs_new(world);
    ecs_add(world, r1, Tag);
    ecs_entity_t r2 = ecs_new(world);
    ecs_add(world, r2, Tag);
    ecs_entity_t p = ecs_new_w_pair(world, EcsChildOf, r1);
    ecs_entity_t c = ecs_new_w_pair(world, EcsChildOf, p);
    ...
    ecs_add_pair(world, p, EcsChildOf, r2);
    test_int(log.count, 4);
    test_uint(log.events[0], EcsOnRemove);
    test_uint(log.entities[0], p);
    test_uint(log.sources[0], r1);
    test_uint(log.events[1], EcsOnRemove);
    test_uint(log.entities[1], c);
    test_uint(log.sources[1], r1);
    test_uint(log.events[2], EcsOnAdd);
    test_uint(log.entities[2], p);
    test_uint(log.sources[2], r2);
    test_uint(log.events[3], EcsOnAdd);
    test_uint(log.entities[3], c);
    test_uint(log.sources[3], r2);
...
void Observer_propagate_remove_first_wildcard_pair(void) {
    ...
    ecs_remove_pair(world, p, Rel, TgtA);
    test_int(log.count, 1);
    test_uint(log.events[0], EcsOnRemove);
    test_uint(log.event_ids[0], ecs_pair(Rel, TgtA));
    test_uint(log.entities[0], c);
    test_uint(log.sources[0], p);
...
void Observer_propagate_add_second_wildcard_pair(void) {
    ...
    test_int(log.count, 1);
    test_uint(log.events[0], EcsOnAdd);
    test_uint(log.event_ids[0], ecs_pair(Rel, TgtB));
    test_uint(log.entities[0], c);
    test_uint(log.sources[0], p);
...
void Observer_propagate_isa_add_to_base(void) {
    ...
    Probe ctx = {0};
    ecs_observer(world, {
        .query.terms = {{ Tag, .src.id = EcsUp, .trav = EcsIsA }},
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    });
...
void Observer_propagate_isa_remove_from_base_w_second_base(void) {
    ...
    ecs_add(world, b1, Tag);
    ecs_entity_t b2 = ecs_new(world);
    ecs_add(world, b2, Tag);
    ecs_entity_t inst = ecs_new_w_pair(world, EcsIsA, b1);
    ecs_add_pair(world, inst, EcsIsA, b2);
    ...
    ecs_remove(world, b1, Tag);
    test_int(log.count, 1);
    test_uint(log.events[0], EcsOnRemove);
    test_uint(log.entities[0], inst);
    test_uint(log.sources[0], b1);
...
void Observer_propagate_isa_masked_add_to_base_of_base(void) {
    ...
    ecs_entity_t b2 = ecs_new(world);
    ecs_entity_t b1 = ecs_new_w_pair(world, EcsIsA, b2);
    ecs_add(world, b1, Tag);
    ecs_entity_t inst = ecs_new_w_pair(world, EcsIsA, b1);
    ecs_add_id(world, inst, ecs_new(world));
    ...
    ecs_add(world, b2, Tag);
    test_int(ctx.invoked, 1);
    test_uint(ctx.e[0], b1);
    test_uint(ctx.s[0][0], b2);
...
void Observer_propagate_isa_add_to_base_of_base(void) {
    ...
    test_int(ctx.invoked, 2);
    test_uint(ctx.e[0], b1);
    test_uint(ctx.e[1], inst);
    test_uint(ctx.s[0][0], b2);
    test_uint(ctx.s[1][0], b2);
...
void Observer_propagate_custom_rel_add_to_target(void) {
    ...
    ecs_entity_t tgt = ecs_new(world);
    ecs_entity_t e = ecs_new_w_pair(world, R, tgt);

    Probe ctx = {0};
    ecs_observer(world, {
        .query.terms = {{ Tag, .src.id = EcsUp, .trav = R }},
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    });

    ecs_add(world, tgt, Tag);
    test_int(ctx.invoked, 1);
    test_uint(ctx.e[0], e);
    test_uint(ctx.s[0][0], tgt);
...
void Observer_propagate_custom_rel_masked_add(void) {
    ...
    ecs_entity_t t2 = ecs_new(world);
    ecs_entity_t t1 = ecs_new_w_pair(world, R, t2);
    ecs_add(world, t1, Tag);
    ecs_entity_t e = ecs_new_w_pair(world, R, t1);
    ...
    ecs_add(world, t2, Tag);
    test_int(ctx.invoked, 1);
    test_uint(ctx.e[0], t1);
    test_uint(ctx.s[0][0], t2);
...
void Observer_multi_term_inheritable_component_owned(void) {
    ...
    ecs_observer(world, {
        .query.terms = {
            { ecs_id(Position) },
            { ecs_id(Velocity), .inout = EcsInOutNone }
        },
        .events = {EcsOnSet},
        .callback = Observer,
        .ctx = &ctx
...
void Observer_nomatch_table_memo_alternating_tables(void) {
    ...
    ecs_entity_t skip = ecs_new_w(world, TagA);
...
void Observer_up_isa_term_alternating_match_and_no_match(void) {
    ...
    ecs_entity_t with_pos = ecs_new_w_id(world, EcsPrefab);
    ecs_set(world, with_pos, Position, {10, 20});
    ecs_entity_t without_pos = ecs_new_w_id(world, EcsPrefab);
...
void Observer_up_isa_not_term_base_gains_component(void) {
    ...
    ecs_observer(world, {
        .query.terms = {
            { TagA, .src.id = EcsSelf },
            { TagB, .src.id = EcsUp, .trav = EcsIsA, .oper = EcsNot }
        },
        .events = {EcsOnAdd},
        .callback = Observer,
        .ctx = &ctx
    ...
    ecs_add(world, e3, TagA);
    test_int(ctx.invoked, 1);
    test_uint(ctx.e[0], e3);

    ecs_fini(world);
}

static int propagate_gate_self_count = 0;
static int propagate_gate_up_count = 0;

static void PropagateGateSelfObserver(ecs_iter_t *it) {
    propagate_gate_self_count += it->count;
}

static void PropagateGateUpObserver(ecs_iter_t *it) {
