...
void ecs_table_deinit(
    ...
    ecs_table_t *table)
{
    int32_t count = ecs_vector_count(table->columns[0].data);
    if (count) {
        ecs_notify(
    ...
    int32_t sindex)
{
    ...
    int32_t column_last = ecs_vector_count(table->type) + 1;
    int32_t i;
    ...
    int32_t count)
{
    ...
    int32_t size = ecs_vector_set_size(
...
{    
    ...
    if (!row_ptr_1) {
        row_ptr_1 = ecs_map_get(stage->entity_index, ecs_row_t, e1);
    }

    if (!row_ptr_2) {
        row_ptr_2 = ecs_map_get(stage->entity_index, ecs_row_t, e2);
    }
    ...
    int32_t count)
{
    ...
    for (i = 0; i < count; i ++) {
        ...
        ecs_row_t *row_ptr = ecs_map_get(stage->entity_index, ecs_row_t, cur);
        row_ptr->index = row + i;
    }

    entities[row + count - 1] = e;
    ecs_row_t *row_ptr = ecs_map_get(stage->entity_index, ecs_row_t, e);
