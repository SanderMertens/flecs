    ...
    ecs_component_record_t *cdr)
{
    ...
    if (cdr->flags & (EcsIdIsSparse|EcsIdDontFragment)) {
        ecs_entity_t entity = ecs_table_entities(table)[row];
        return (flecs_component_ptr_t){
            .ti = cdr->type_info,
            .ptr = flecs_component_sparse_get(cdr, entity)
        };
    }
...
void* flecs_get_component(
    ...
    ecs_component_record_t *cdr)
{
    return flecs_get_component_ptr(table, row, cdr).ptr;
}
    ...
    do {
        ...
        if (!tr) {
            if (cdr->flags & EcsIdDontFragment) {
                ptr = flecs_component_sparse_get(cdr, base);
            }

            if (!ptr) {
                ptr = flecs_get_base_component(world, table, id, cdr, 
                    recur_depth + 1);
            }
        } else {
            if (cdr->flags & EcsIdIsSparse) {
                return flecs_component_sparse_get(cdr, base);
            } else {
    ...
    int32_t count)
{
    ...
    ecs_component_record_t *union_cdr = flecs_components_get(world, 
        ecs_pair(EcsWildcard, EcsUnion));
    ecs_assert(union_cdr != NULL, ECS_INTERNAL_ERROR, NULL);
    const ecs_table_record_t *tr = flecs_component_get_table(
        union_cdr, base_table);
    ...
    do {
        ...
        if (ECS_PAIR_SECOND(id) == EcsUnion) {
            ...
            for (j = row; j < (row + count); j ++) {
                flecs_switch_set(cdr->sparse, (uint32_t)entities[j], tgt);
            }
    ...
    const ecs_instantiate_ctx_t *ctx)
{
    ...
    ecs_component_record_t *cdr = flecs_components_get(world, ecs_childof(base));
    ...
    bool construct)
{
    ...
    for (i = 0; i < added->count; i ++) {
        ...
        if (cdr && cdr->flags & EcsIdIsSparse) {
            int32_t sparse_count = flecs_sparse_count(cdr->sparse);

            for (j = 0; j < count; j ++) {
                if (construct) {
                    flecs_component_sparse_insert(world, cdr, table, row + j);
                } else {
                    flecs_component_sparse_emplace(world, cdr, table, row + j);
                }
            }

            if (cdr->flags & EcsIdDontFragment) {
                is_new |= sparse_count != flecs_sparse_count(cdr->sparse);
            }
        }
    }
    ...
    const ecs_type_t *removed)
{
    ...
    for (i = 0; i < removed->count; i ++) {
        ...
        if (cdr && cdr->flags & EcsIdIsSparse) {
            for (j = 0; j < count; j ++) {
                flecs_component_sparse_remove(world, cdr, table, row + j);
            }
        }
    }
}
    ...
    for (i = 0; i < removed->count; i ++) {
        ...
        if (cdr && cdr->flags & EcsIdDontFragment) {
            ...
            for (j = 0; j < count; j ++) {
                ...
                if (flecs_component_sparse_get(cdr, e)) {
                    return true;
                }
            }
        }
        ...
        if (ECS_IS_PAIR(id)) {
            ...
            if (cdr && cdr->flags & EcsIdIsUnion) {
                ...
                for (j = 0; j < count; j ++) {
                    ecs_entity_t e = entities[row + j];
                    flecs_switch_set(
                        cdr->sparse, (uint32_t)e, ecs_pair_second(world, id));
                }
            }
        }
    }
    ...
    for (i = 0; i < removed->count; i ++) {
        ...
        if (ECS_IS_PAIR(id)) {
            ...
            if (cdr && cdr->flags & EcsIdIsUnion) {
                ...
                for (j = 0; j < count; j ++) {
                    ecs_entity_t e = entities[row + j];
                    flecs_switch_reset(cdr->sparse, (uint32_t)e);
                }
            }
        }
    }
    ...
    if (component_data) {
        ...
        for (c_i = 0; c_i < component_ids->count; c_i ++) {
            ...
            if (cdr->flags & EcsIdIsSparse) {
                int32_t e;
                for (e = 0; e < count; e ++) {
                    ptr = flecs_component_sparse_get(cdr, entities[e]);
    ...
    ecs_record_t *r)
{
    ...
    if (id < FLECS_HI_COMPONENT_ID) {
        ...
        if (column_index > 0) {
            ...
            if (cdr->flags & EcsIdIsSparse) {
                dst.ptr = flecs_component_sparse_get(cdr, entity);
                dst.ti = cdr->type_info;
                return dst;
            }
        }
    } else {
        cdr = flecs_components_get(world, id);
        dst = flecs_get_component_ptr(table, ECS_RECORD_TO_ROW(r->row), cdr);
        if (dst.ptr) {
            return dst;
        }
    }
    ...
    dst = flecs_get_component_ptr(r->table, ECS_RECORD_TO_ROW(r->row), cdr);
error:
    return dst;
}
    ...
    ecs_iter_action_t hook)
{
    ...
    if (!tr) {
        dummy_tr.hdr.cache = ECS_CONST_CAST(ecs_table_cache_t*, cdr);
    ...
    ecs_type_t *ids)
{
    ...
    for (i = 0; i < ids->count; i ++) {
        ...
        ecs_component_record_t *cdr = flecs_components_get(world, id);
        dont_fragment |= (cdr->flags & EcsIdDontFragment) != 0;
        ecs_assert(cdr != NULL, ECS_INTERNAL_ERROR, NULL);
        const ecs_type_info_t *ti = cdr->type_info;
        ...
        flecs_component_get_table(cdr, table);
        if (!tr) {
            dummy_tr.hdr.cache = (ecs_table_cache_t*)cdr;
            dummy_tr.hdr.table = table;
            dummy_tr.column = -1;
            dummy_tr.index = -1;
            dummy_tr.count = 0;
        ...
        if (cdr->flags & EcsIdIsSparse) {
            ...
            if (on_set) {
                flecs_invoke_hook(world, table, cdr, tr, count, row, 
    ...
    bool invoke_hook)
{
    ...
    ecs_component_record_t *cdr = flecs_components_get(world, id);
    ecs_assert(cdr != NULL, ECS_INTERNAL_ERROR, NULL);
    bool dont_fragment = (cdr->flags & EcsIdDontFragment) != 0;
    ...
    bool name_assigned)
{
    ...
    if (desc->set) {
        ...
        while ((void)(v = &desc->set[i ++]), v->type) {
            ...
            ecs_component_record_t *cdr = flecs_components_get(world, v->type);
            flecs_component_ptr_t ptr = flecs_get_component_ptr(table, row, cdr);
    ...
    ecs_table_t *table)
{
    ...
    for (i = 0; i < count; i ++) {
        ...
        if (flags & EcsEntityIsId) {
            if ((cdr = flecs_components_get(world, e))) {
                flecs_id_mark_for_delete(world, cdr, 
                    ECS_ID_ON_DELETE(cdr->flags), true);
            }
            if ((cdr = flecs_components_get(world, ecs_pair(e, EcsWildcard)))) {
                flecs_id_mark_for_delete(world, cdr, 
                    ECS_ID_ON_DELETE(cdr->flags), true);
            }
        }
        if (flags & EcsEntityIsTarget) {
            if ((cdr = flecs_components_get(world, ecs_pair(EcsWildcard, e)))) {
                flecs_id_mark_for_delete(world, cdr, 
                    ECS_ID_ON_DELETE_TARGET(cdr->flags), true);
            }
            if ((cdr = flecs_components_get(world, ecs_pair(EcsFlag, e)))) {
                flecs_id_mark_for_delete(world, cdr, 
                    ECS_ID_ON_DELETE_TARGET(cdr->flags), true);
            }
        }
    ...
    bool delete_target)
{
    ...
    if (!result && delete_target) {
        ...
        do {
            ...
            ecs_component_record_t *cdrr = (ecs_component_record_t*)trr->hdr.cache;
            result = ECS_ID_ON_DELETE_TARGET(cdrr->flags);
            if (result == EcsDelete) {
...
void flecs_id_mark_for_delete(
    ...
    bool delete_id)
{
    if (cdr->flags & EcsIdMarkedForDelete) {
        return;
    }

    cdr->flags |= EcsIdMarkedForDelete;
    flecs_marked_id_push(world, cdr, action, delete_id);
    ...
    if (flecs_table_cache_empty_iter(&cdr->cache, &it)) {
        const ecs_table_record_t *tr;
        while ((tr = flecs_table_cache_next(&it, ecs_table_record_t))) {
    ...
    bool delete_id)
{
    ...
    if (!action) {
        ...
        if (!ecs_id_is_pair(id) || ECS_PAIR_SECOND(id) == EcsWildcard) {
            ...
            action = ECS_ID_ON_DELETE(cdr->flags);
        }
    }
    ...
    flecs_id_mark_for_delete(world, cdr, action, delete_id);
    ...
    ecs_table_t *table) 
{
    ...
    const ecs_table_record_t *tr;
    for (i = 0; i < count; i ++) {
        const ecs_component_record_t *cdr = ids[i].cdr;

        if (!(tr = flecs_component_get_table(cdr, dst_table))) {
            continue;
        }
        ...
        do {
            ...
            world, dst_table, t, cdr->id, NULL)) != -1);
    }
    ...
    ecs_world_t *world)
{
    ...
    do {
        for (i = last - 1; i >= first; i --) {
            ecs_component_record_t *cdr = ids[i].cdr;
    ...
    ecs_world_t *world)
{
    ...
    do {
        for (i = 0; i < count; i ++) {
            ...
            flecs_component_release_tables(world, cdr);
            ...
            int32_t rc = flecs_component_release(world, cdr);
    ...
    if (copy_value) {
        ...
        if (dst_table->flags & EcsTableHasSparse) {
            ...
            for (i = 0; i < count; i ++) {
                ...
                ecs_component_record_t *cdr = (ecs_component_record_t*)tr->hdr.cache;
                if (cdr->sparse) {
                    void *src_ptr = flecs_component_sparse_get(cdr, src);
                    if (src_ptr) {
                        ecs_set_id(world, dst, cdr->id, 0, src_ptr);
                    }
                }
            }
        }
...
#define ecs_get_low_id(table, r, id)\
    ...
    if (column_index > 0) {\
        ...
        ecs_component_record_t *cdr = (ecs_component_record_t*)tr->hdr.cache;\
        if (cdr->flags & EcsIdIsSparse) {\
            return flecs_component_sparse_get(cdr, entity);\
        }\
    }
    ...
    ecs_id_t id)
{
    ...
    if (cdr->flags & EcsIdDontFragment) {
        void *ptr = flecs_component_sparse_get(cdr, entity);
        if (ptr) {
            return ptr;
        }
    }
    ...
    const ecs_table_record_t *tr = flecs_component_get_table(cdr, table);
    if (!tr) {
        return flecs_get_base_component(world, table, id, cdr, 0);
    } else {
        if (cdr->flags & EcsIdIsSparse) {
            return flecs_component_sparse_get(cdr, entity);
        }
    ...
    ecs_id_t id)
{
    const ecs_world_t *world = ecs_get_world(stage);
    ecs_component_record_t *cdr = flecs_components_get(world, id);
    return flecs_get_component(r->table, ECS_RECORD_TO_ROW(r->row), cdr);
}
    ...
    bool *is_new)
{
    ...
    void *ptr = flecs_get_component(r->table, ECS_RECORD_TO_ROW(r->row), cdr);
    ...
    bool invoke_hook)
{
    ...
    ecs_component_record_t *cdr = flecs_components_get(world, id);
    if (!cdr || !flecs_component_get_table(cdr, table)) {
        flecs_defer_end(world, stage);
        return;
    }
    ...
    ecs_id_t id)
{
    ...
    return (cdr->flags & EcsIdCanToggle) != 0;
}
#endif
    ...
    ecs_id_t id)
{
    ...
    if (ECS_IS_PAIR(id) && (table->flags & EcsTableHasUnion)) {
        ecs_component_record_t *u_cdr = flecs_components_get(world, 
            ecs_pair(ECS_PAIR_FIRST(id), EcsUnion));
        if (u_cdr && u_cdr->flags & EcsIdIsUnion) {
    ...
    int32_t index)
{
    ...
    if (result == EcsUnion) {
        ...
        ecs_component_record_t *wc_cdr = flecs_components_get(world, wc);
        ecs_assert(wc_cdr != NULL, ECS_INTERNAL_ERROR, NULL);
        result = flecs_switch_get(wc_cdr->sparse, (uint32_t)entity);
    }
        ...
        if (!flecs_entities_is_valid(world, rel)) {
            ...
            *id_out = 0;
            ...
            if (!flecs_entities_is_valid(world, obj)) {
                ...
                if (cdr) {
                    ecs_entity_t action = ECS_ID_ON_DELETE_TARGET(cdr->flags);
                    if (action == EcsDelete) {
    ...
    int32_t start)
{
    ...
    do {
        ...
        if (id) {
            cdr = flecs_components_get(world, cmd->id);
            if (cdr && cdr->flags & EcsIdDontFragment) {
                continue;
            }
    ...
    if (set_mask[0] | set_mask[1] | set_mask[2] | set_mask[3]) {
        ...
        do {
            ...
            case EcsCmdEnsure: {
                ...
                if (r->table) {
                    ecs_component_record_t *cdr = flecs_components_get(world, cmd->id);
                    ptr = flecs_get_component_ptr(
                        r->table, ECS_RECORD_TO_ROW(r->row), cdr);
                }
