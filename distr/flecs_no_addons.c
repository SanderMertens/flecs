    ...
    ecs_entity_t e)
{
    ...
    ecs_vec_t *cmds = &commands->queue;
    int32_t cur = ecs_vec_count(cmds);
    if (!cur) {
        commands->entity = e;
        commands->entry.first = -1;
        commands->second_entity = 0;
    }
    ecs_cmd_entry_t *entry;
    if (e == commands->entity) {
        entry = &commands->entry;
    } else if (e == commands->second_entity) {
        entry = &commands->second_entry;
    } else if (cur == 1) {
        commands->second_entity = e;
        commands->second_entry.first = -1;
        entry = &commands->second_entry;
    } else {
        entry = flecs_sparse_get_t(&commands->entries, ecs_cmd_entry_t, e);
    }
    ...
    ecs_stage_t *stage)
{
    ...
    ecs_assert(cmd->next == NULL, ECS_INTERNAL_ERROR, NULL);
    cmd->next = ecs_os_calloc_t(ecs_commands_t);
    flecs_commands_init(stage, cmd->next);
    cmd->next->prev = cmd;
}
    ...
    bool deferred)
{
    ...
    flecs_check_exclusive_world_access_write(stage->world);
    flecs_commands_begin(stage->world, stage);
    return false;
}
...
{
    ecs_commands_t *commands = stage->cmd;
    ecs_cmd_entry_t *entry = entity == commands->entity ? &commands->entry :
        entity == commands->second_entity ? &commands->second_entry :
        flecs_sparse_get_t(&commands->entries, ecs_cmd_entry_t, entity);
    ...
    ecs_stage_t *stage)
{
    ...
    ecs_commands_t *commands = stage->cmd;
    if (!ecs_vec_count(&commands->queue)) {
        return false;
    }
    flecs_check_exclusive_world_access_write(world);
    ecs_stage_t *dst_stage = world->stages[0];
    ecs_commands_t *root = commands;
    ecs_vec_t *queue;
    ecs_cmd_t *cmds;
    int32_t i, count;
...
enter:
    ...
    flecs_commands_begin(world, stage);
    if (dst_stage != stage) {
        flecs_commands_begin(world, dst_stage);
    }
    ...
    cmds = ecs_vec_first(queue);
    count = ecs_vec_count(queue);
    i = 0;

resume:
    for (; i < count; i ++) {
        ecs_cmd_t *cmd = &cmds[i];
        if (cmd->kind == EcsCmdSkip && !cmd->is._1.value) {
            if (cmd->entry) {
                cmd->entry->first = -1;
            }
        ...
        if (cmd->next_for_entity < 0) {
            ...
            if (is_alive) {
                if (!diff_builder_used) {
                    ...
                    ecs_vec_init_t(&world->allocator, &set_ids, ecs_id_t, 0);
        ...
        case EcsCmdAdd:
            ...
            break;
        case EcsCmdRemove:
            flecs_remove_id_now(world, e, id);
        ...
        case EcsCmdPath: {
            ...
            if (cmd->id) {
                if (ecs_is_alive(world, cmd->id)) {
                    if (ecs_get_parent(world, e) != cmd->id) {
...
next:
        if (dst_stage != stage && ecs_vec_count(&dst_stage->cmd->queue)) {
            flecs_commands_flush(world, dst_stage);
        }
        if (ecs_vec_count(&stage->cmd->queue)) {
            commands->cursor = i + 1;
            commands = stage->cmd;
            goto enter;
        }
    ...
    if (dst_stage != stage) {
        flecs_commands_end(world, dst_stage);
    }
    stage->cmd = commands;
...
{
    ecs_commands_t *cmd = stage->cmd;
    ecs_assert(cmd->prev != NULL, ECS_INTERNAL_ERROR, NULL);
    if (ecs_vec_count(&cmd->queue)) {
    ...
    ecs_stage_t *stage)
{
    ...
    if (ecs_vec_count(&commands)) {
        ...
        stage->cmd->entry.first = -1;
        stage->cmd->second_entry.first = -1;
    }
    ...
    void *ptr)
{
    ...
    it.trs = &tr;
    it.columns = &column;
    it.row_fields = !!(cr->flags & EcsIdSparse);
    ...
    ecs_type_t *ids)
{
    ...
    for (i = 0; i < ids->count; i ++) {
        ...
        if (!(cr->flags & EcsIdSparse)) {
            ...
            ecs_assert(elem->column != -1, ECS_INTERNAL_ERROR, NULL);
            ecs_assert(elem->tr->count == 1, ECS_INTERNAL_ERROR, NULL);
            column = elem->column;
        }

        if (on_validate) {
            bool emit = ((cr->flags & EcsIdDontFragment) != 0) ||
            ...
            for (j = 0; j < count; j ++) {
                ...
                if (emit) {
                    ecs_type_t single = { .array = &id, .count = 1 };
                    flecs_emit(world, world, &(ecs_event_desc_t) {
                        .event = EcsOnSet,
                        .ids = &single,
                        .table = table,
    ...
    void *ptr)
{
    ...
    if (on_validate) {
        ...
        if (!on_validate(world, entities[0], ptr)) {
            return;
        }
    }
        ...
        if (on_set) {
            ...
            if (!(cr->flags & EcsIdSparse)) {
                const ecs_table_cache_elem_t *elem =
                    flecs_table_cache_get_elem(&cr->cache, table);
                ecs_assert(elem != NULL, ECS_INTERNAL_ERROR, NULL);
                ecs_assert(elem->column != -1, ECS_INTERNAL_ERROR, NULL);
                ecs_assert(elem->tr->count == 1, ECS_INTERNAL_ERROR, NULL);
                column = elem->column;
            ...
            flecs_invoke_hook(world, table, cr, column, 1, row,
        ...
        for (c_i = 0; c_i < component_ids->count; c_i ++) {
            ...
            if (cr->flags & EcsIdSparse) {
                ...
                for (e = 0; e < count; e ++) {
                    ...
                    if (is_move) {
                        flecs_type_info_move(ptr, src_ptr, 1, ti);
                    } else {
                ...
                if (is_move) {
                    flecs_type_info_move(ptr, src_ptr, count, ti);
                } else {
                    flecs_type_info_copy(ptr, src_ptr, count, ti);
                }
            }
        };
    ...
    bool enable)
{
    ...
    ecs_bitset_t *bs = flecs_table_get_toggle(table, component);
...
static ecs_event_id_record_t* flecs_event_id_record_get_for(
    const ecs_observable_t *observable,
    ecs_id_t id,
    ecs_entity_t event)
{
    ...
    return flecs_event_id_record_get(er, id);
}
...
bool flecs_observers_exist(
    ...
    ecs_entity_t event)
{
    ecs_event_id_record_t *ider = flecs_event_id_record_get_for(
        observable, id, event);
    if (!ider) {
        return false;
    }

    return (ider->observer_count - ider->up_notify_count) > 0;
}
    ...
    ecs_table_range_t *range)
{
    ...
    for (ider_i = 0; ider_i < ider_count; ider_i ++) {
        ...
        if (count) {
            ...
            if (!owned) {
                flecs_observers_invoke_up_notify(
    ...
    if (table->_->traversable_count) {
        ...
        if (i != it->count) {
            ...
            for (i = 0; i < count; i ++) {
                ...
                it->trs[0] = tr;
                ECS_CONST_CAST(int16_t*, it->columns)[0] = tr ? tr->column : -1;
    ...
    ecs_table_t *table)
{
    ...
    ECS_CONST_CAST(int16_t*, it->columns)[0] =
        it->trs[0] ? it->trs[0]->column : -1;
    it->sources[0] = 0;
...
static ecs_flags32_t flecs_type_hooks_specified(
    const ecs_type_hooks_t *hooks,
    bool exclude_illegal)
{
    ...
    const ecs_type_hooks_t *h)
{
    ...
    ecs_check(!conflicts, ECS_INVALID_PARAMETER,
        ...
        flecs_errstr(ecs_get_path(world, component)), conflicts);
    ...
    ecs_type_hooks_t hooks = prev_hooks;
    flags |= (prev_hooks.flags & ECS_TYPE_HOOKS_ILLEGAL) &
        ~flecs_type_hooks_specified(h, false);
    ...
    if (h->ctor) hooks.ctor = h->ctor;
    if (h->dtor) hooks.dtor = h->dtor;
    if (h->copy) hooks.copy = h->copy;
    if (h->move) hooks.move = h->move;
    if (h->copy_ctor) hooks.copy_ctor = h->copy_ctor;
    if (h->move_ctor) hooks.move_ctor = h->move_ctor;
    if (h->ctor_move_dtor) hooks.ctor_move_dtor = h->ctor_move_dtor;
    if (h->move_dtor) hooks.move_dtor = h->move_dtor;
    if (h->cmp) hooks.cmp = h->cmp;
    if (h->equals) hooks.equals = h->equals;

    if (h->on_add) hooks.on_add = h->on_add;
    if (h->on_remove) hooks.on_remove = h->on_remove;
    if (h->on_set) hooks.on_set = h->on_set;
    if (h->on_replace) hooks.on_replace = h->on_replace;
    if (h->on_validate) hooks.on_validate = h->on_validate;

    if (h->ctx) hooks.ctx = h->ctx;
    if (h->binding_ctx) hooks.binding_ctx = h->binding_ctx;
    if (h->lifecycle_ctx) hooks.lifecycle_ctx = h->lifecycle_ctx;
    if (h->ctx_free) hooks.ctx_free = h->ctx_free;
    if (h->binding_ctx_free) hooks.binding_ctx_free = h->binding_ctx_free;
    if (h->lifecycle_ctx_free) hooks.lifecycle_ctx_free = h->lifecycle_ctx_free;
    ...
    if (!h->copy_ctor && !(flags & ECS_TYPE_HOOK_COPY_CTOR_ILLEGAL)) {
        if (h->copy) {
            hooks.copy_ctor = flecs_default_copy_ctor;
        }
    }

    if (!h->move_ctor && !(flags & ECS_TYPE_HOOK_MOVE_CTOR_ILLEGAL)) {
        if (h->move) {
            hooks.move_ctor = flecs_default_move_ctor;
        }
    }
    ...
    if (!h->move_dtor) {
        ...
        if (h->move) {
            ...
            if (h->dtor) {
                illegal_check |= ECS_TYPE_HOOK_DTOR_ILLEGAL;
                hooks.move_dtor = flecs_default_move_w_dtor;
            } else {
                hooks.move_dtor = flecs_default_move;
            }
        } else {
            if (h->dtor) {
                illegal_check |= ECS_TYPE_HOOK_DTOR_ILLEGAL;
    ...
    if (!hooks.cmp || hooks.cmp == flecs_comp_illegal) {
        flags |= ECS_TYPE_HOOK_CMP_ILLEGAL;
    }

    if (!hooks.equals || hooks.equals == flecs_equals_illegal) {
        if(flags & ECS_TYPE_HOOK_CMP_ILLEGAL) {
            flags |= ECS_TYPE_HOOK_EQUALS_ILLEGAL;
        } else {
            hooks.equals = flecs_default_equals;
            flags &= ~ECS_TYPE_HOOK_EQUALS_ILLEGAL;
        }
    }

    if(flags & ECS_TYPE_HOOK_CTOR_ILLEGAL) hooks.ctor = flecs_ctor_illegal;
    if(flags & ECS_TYPE_HOOK_DTOR_ILLEGAL) hooks.dtor = flecs_dtor_illegal;
    if(flags & ECS_TYPE_HOOK_COPY_ILLEGAL) hooks.copy = flecs_copy_illegal;
    if(flags & ECS_TYPE_HOOK_MOVE_ILLEGAL) hooks.move = flecs_move_illegal;
    if(flags & ECS_TYPE_HOOK_CMP_ILLEGAL) hooks.cmp = flecs_comp_illegal;
    if(flags & ECS_TYPE_HOOK_EQUALS_ILLEGAL) hooks.equals = flecs_equals_illegal;

    if(flags & ECS_TYPE_HOOK_COPY_CTOR_ILLEGAL) {
        hooks.copy_ctor = flecs_copy_ctor_illegal;
    }

    if(flags & ECS_TYPE_HOOK_MOVE_CTOR_ILLEGAL) {
        hooks.move_ctor = flecs_move_ctor_illegal;
    }

    if(flags & ECS_TYPE_HOOK_CTOR_MOVE_DTOR_ILLEGAL) {
        hooks.ctor_move_dtor = flecs_move_ctor_illegal;
    }

    if(flags & ECS_TYPE_HOOK_MOVE_DTOR_ILLEGAL) {
        hooks.move_dtor = flecs_move_ctor_illegal;
    }

    hooks.flags = flags;

    if (hooks.ctor) hooks.flags |= ECS_TYPE_HOOK_CTOR;
    if (hooks.dtor) hooks.flags |= ECS_TYPE_HOOK_DTOR;
    if (hooks.move) hooks.flags |= ECS_TYPE_HOOK_MOVE;
    if (hooks.move_ctor) hooks.flags |= ECS_TYPE_HOOK_MOVE_CTOR;
    if (hooks.ctor_move_dtor) hooks.flags |= ECS_TYPE_HOOK_CTOR_MOVE_DTOR;
    if (hooks.move_dtor) hooks.flags |= ECS_TYPE_HOOK_MOVE_DTOR;
    if (hooks.copy) hooks.flags |= ECS_TYPE_HOOK_COPY;
    if (hooks.copy_ctor) hooks.flags |= ECS_TYPE_HOOK_COPY_CTOR;
    if (hooks.cmp) hooks.flags |= ECS_TYPE_HOOK_CMP;
    if (hooks.equals) hooks.flags |= ECS_TYPE_HOOK_EQUALS;

    hooks.flags |= prev_hooks.flags & ECS_TYPE_HOOK_IN_USE;

    if (in_use && !flecs_type_hooks_storage_equal(&hooks, &prev_hooks)) {
    ...
    ti->hooks = hooks;
    if (component < FLECS_HI_COMPONENT_ID) {
        if (hooks.on_set || hooks.copy || hooks.move || hooks.on_replace || hooks.on_validate) {
    ...
    bool *is_new)
{
    ...
    if (dense && dense < sparse->count) {
        ...
        if (!dense) {
            dense = ecs_vec_count(&sparse->dense);
            ecs_vec_append_t(sparse->allocator, &sparse->dense, uint64_t);
            uint64_t *ids = ecs_vec_first_t(&sparse->dense, uint64_t);
            flecs_sparse_assign_index(page, ids, id, dense);
            if (index >= sparse->max_id) {
                sparse->max_id = index;
            }
        }
...
static bool flecs_sparse_remove_id(
    ecs_sparse_t *sparse,
    ecs_size_t size,
    uint64_t id,
    bool generation)
{
    ...
    if (generation) {
        uint64_t *ids = ecs_vec_first_t(&sparse->dense, uint64_t);
        ecs_assert(ids[dense] == id, ECS_INVALID_PARAMETER, NULL);
        ids[dense] = ECS_GENERATION_INC(id);
        if (dense >= sparse->count) {
            return false;
        }
    }

    if (dense < sparse->count) {
        int32_t last = -- sparse->count;
        if (dense != last) {
            flecs_sparse_swap_dense(sparse, page, dense, last);
        }
    }

    if (sparse->size) {
        ecs_os_memset(DATA(page->data, sparse->size, offset), 0, sparse->size);
    }
...
bool flecs_sparse_remove(
    ...
    uint64_t id)
{
    return flecs_sparse_remove_id(sparse, size, id, false);
}
...
bool flecs_sparse_remove_w_gen(
    ...
    uint64_t id)
{
    return flecs_sparse_remove_id(sparse, size, id, true);
}
    ...
    const ecs_query_desc_t *const_desc)
{
    ...
    if (flecs_query_set_caching_policy(result, &desc)) {
        goto error;
    }
...
void flecs_entity_index_remove(
    ...
    uint64_t entity)
{
    ecs_record_t *r = flecs_entity_index_remove_intern(index, entity);
    if (!r) {
        /* Entity was not alive, nothing else to be done. */
        return;
    }
    ...
    const ecs_table_t *table)
{
    ...
    elem->count --;
    world->non_fragmenting_child_count --;
    ecs_assert(world->non_fragmenting_child_count >= 0,
        ECS_INTERNAL_ERROR, NULL);
    ...
    ecs_entity_t entity)
{
    ...
    flecs_remove_non_fragmenting_child_from_table(world, cr, table);
    ...
    int32_t count)
{
    ...
    for (; i < end; i ++) {
        ...
        if (src && (src->flags & EcsTableHasParent)) {
            flecs_remove_non_fragmenting_child_from_table(world, cr, src);
        }
...
static ecs_flags32_t flecs_type_info_flags(
    const ecs_type_info_t *ti) 
{
...
static void flecs_table_init_columns(
    ecs_world_t *world,
    ecs_table_t *table,
...
static void flecs_table_init_data(
    ecs_world_t *world,
    ecs_table_t *table)
{
...
static void flecs_table_init_flags(
    ecs_world_t *world,
    ecs_table_t *table)
{
    ecs_id_t *ids = table->type.array;
...
static void flecs_table_records_unregister(
    ecs_world_t *world,
    ecs_table_t *table)
{
...
static void flecs_table_invoke_hook(
    ecs_world_t *world,
    ecs_table_t *table,
...
static void flecs_table_invoke_ctor(
    ecs_world_t *world,
    ecs_table_t *table,
    int32_t column_index,
    int32_t row,
    int32_t count)
{
    ...
    ecs_table_overrides_t *o = table->data.overrides;
    ecs_ref_t *r = o ? &o->refs[column_index] : NULL;
    if (r && r->entity) {
        ...
        void *base_ptr = ecs_ref_get_id(world, r, table->type.array[record_index]);
...
/* Destruct components */
static void flecs_table_invoke_dtor(
    ecs_column_t *column,
    int32_t row,
    int32_t count)
{
...
static void flecs_table_invoke_add_hooks(
    ecs_world_t *world,
    ecs_table_t *table,
    int32_t column_index,
    ecs_entity_t *entities,
...
static void flecs_table_invoke_remove_hooks(
    ecs_world_t *world,
    ecs_table_t *table,
    ecs_column_t *column,
    ecs_entity_t *entities,
...
static void flecs_table_remove_dont_fragment(
    ecs_world_t *world,
    ecs_entity_t e)
{
    ecs_record_t *r = flecs_entities_get(world, e);
...
static void flecs_table_dtor_all(
    ecs_world_t *world,
    ecs_table_t *table)
{
...
static void flecs_table_mark_table_dirty(
    ecs_world_t *world,
    ecs_table_t *table,
    int32_t index)
{
...
static void flecs_table_resize_column(
    ecs_column_t *column,
    int32_t count,
    int32_t old_size,
    int32_t new_size)
{
    ...
    if (!new_size) {
        column->data = NULL;
    } else if (count && (ti->hooks.ctor_move_dtor || new_size < old_size)) {
        void *ptr = ecs_os_malloc(ti->size * new_size);
        flecs_type_info_ctor_move_dtor(ptr, old, count, ti);
        column->data = ptr;
    } else {
    ...
    bool on_add)
{
    ...
    for (int32_t i = 0; i < table->column_count; i ++) {
        ecs_column_t *column = &table->data.columns[i];
        const ecs_type_info_t *ti = column->ti;
        flecs_table_resize_column(column, count, prev_size, entities.size);
        if (construct) {
...
static void flecs_table_copy_elem(
    void *dst_ptr,
    const void *src_ptr,
    ecs_size_t size)
{
...
static void flecs_table_fast_delete(
    ecs_table_t *table,
    int32_t row)
{
    ecs_column_t *columns = table->data.columns;
...
static void flecs_table_fast_move(
    ecs_table_t *dst_table,
    int32_t dst_index,
    ecs_table_t *src_table,
    int32_t src_index)
    ...
    ecs_table_t *table)
{
    ...
    for (int32_t i = 0; i < table->column_count; i ++) {
        flecs_table_resize_column(&columns[i], count, table->data.size, count);
    }
...
static void flecs_table_swap_bitset_columns(
    ecs_table_t *table,
    int32_t row_1,
    int32_t row_2)
{
    ...
    ecs_table_t *src_table)
{
    ...
    for (int32_t src_i = 0; src_i < src_table->column_count; src_i ++) {
            ...
            flecs_column_id(dst_table, dst_i) == flecs_column_id(src_table, src_i))
        {
            ...
            if (dst_count) {
                flecs_table_resize_column(dst, dst_count,
    ...
    ecs_id_t id)
{
    ...
    ecs_id_t *ids = table->type.array;
    int32_t i = table->_->bs_offset, end = table->type.count;
    int32_t column = 0;
...
static DWORD flecs_win_thread(void *ptr) {
    ecs_win_thread_t *thread = ptr;
    thread->callback(thread->arg);
    return 0;
}

static ecs_os_thread_t win_thread_new(
    ecs_os_thread_callback_t callback, 
    void *arg)
{
...
static void win_cond_free(
    ecs_os_cond_t c) 
{
    ecs_os_free((CONDITION_VARIABLE*)(uintptr_t)c);
}

static void win_cond_signal(
    ecs_os_cond_t c) 
{
...
static void win_cond_broadcast(
    ecs_os_cond_t c) 
{
    CONDITION_VARIABLE *cond = (CONDITION_VARIABLE*)(intptr_t)c;
...
static void win_time_setup(void) {
...
static void win_enable_high_timer_resolution(bool enable)
{
...
static uint64_t win_time_now(void) {
    uint64_t now;
...
static ecs_os_dl_t win_dlopen(const char *libname) {
    return (ecs_os_dl_t)(uintptr_t)LoadLibraryA(libname);
}

static ecs_os_proc_t win_dlproc(ecs_os_dl_t lib, const char *procname) {
    union {
        FARPROC p;
        ecs_os_proc_t fn;
    } u;
...
static void win_dlclose(ecs_os_dl_t lib) {
    FreeLibrary((HMODULE)(uintptr_t)lib);
}

static void win_fini(void) {
    ...
    ecs_iter_t *it)
{
    ...
    ecs_assert(flags & EcsQueryIsTrivial, ECS_INTERNAL_ERROR, NULL);
    ecs_assert(flags & EcsQueryMatchOnlySelf, ECS_INTERNAL_ERROR, NULL);
    ecs_assert(!(flags & EcsQueryMatchWildcards), ECS_INTERNAL_ERROR, NULL);

    it->flags |= constrained ? EcsIterTrivialTest :
        (EcsIterTrivialSearch|EcsIterTrivialChangeDetection);
    flecs_query_trivial_set_ids(&ctx);
}
    ...
    ecs_iter_t *it)
{
...

    {
        ...
        if (it->flags & EcsIterTrivialSearch) {
            ecs_query_trivial_ctx_t *op_ctx = &qit->trivial;
            ...
            ecs_abort(ECS_INTERNAL_ERROR,
                "query without plan entered query plan evaluator");
        }
    }
